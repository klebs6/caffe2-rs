/*!
  | Memory format is not the property of
  | a Tensor. It is the way to tell an operator how
  | the result should be organized in memory and
  | nothing more. That means memory format should
  | never be used as return value for any tensor
  | state interrogation functions (internally and
  | externally).
  |
  ...
  |  Preserve:
  |
  |    If any of the input tensors is in
  |    channels_last format, operator output should
  |    be in channels_last format
  |
  |  Contiguous:
  |
  |    Regardless of input tensors format, the
  |    output should be contiguous Tensor.
  |
  |  ChannelsLast:
  |
  |    Regardless of input tensors format, the
  |    output should be in channels_last format.
  */

crate::ix!();

//-------------------------------------------[.cpp/pytorch/c10/core/MemoryFormat.h]
...
/**
  | If you are seeing this, it means that this call
  | site was not checked if the memory format could
  | be preserved, and it was switched to old
  | default behaviour of contiguous
  |
  */
macro_rules! LEGACY_CONTIGUOUS_MEMORY_FORMAT {
    () => {
        /*
...
#[inline] pub fn get_contiguous_memory_format() -> MemoryFormat {
...
/**
  ...
  |
  */
#[inline] pub fn get_channels_last_strides_2d(sizes: &[i32]) -> Vec<i64> {
    
    todo!();
        /*
            vector<int64_t> strides(sizes.size());
      switch (sizes.size()) {
        case 4:
          strides[1] = 1;
          strides[3] = sizes[1];
...
#[inline] pub fn get_channels_last_strides_3d(sizes: &[i32]) -> Vec<i64> {
...
/**
  ...
  |
  */
#[inline] pub fn is_channels_last_strides_2d_s4(
    sizes:   &[i32],
    strides: &[i32]) -> bool {
...
#[inline] pub fn is_channels_last_strides_3d_s5(
    sizes:   &[i32],
    strides: &[i32]) -> bool {
...
/**
  ...
  | `is_channels_last_strides` does the best effort
  ...
  | `is_channels_last_strides` is to serve the
  | objectives:
  |
  ...
  | functions (is_channels_last_strides_*d_s*) for
  | more details.
  */
#[inline] pub fn is_channels_last_strides_2d(
    sizes:   &[i32],
    strides: &[i32]) -> bool {
...
#[inline] pub fn is_channels_last_strides_3d(
    sizes:   &[i32],
    strides: &[i32]) -> bool {
