...
/// CRC64 with Jones coefficients and an init
/// value of 0.
///
impl From<&[u8]> for Crc64 {

    fn from(bytes: &[u8]) -> Self {
        Crc64::from_bytes(bytes)
    }
}

impl From<u64> for Crc64 {

    fn from(checksum: u64) -> Self {
        Crc64::from_checksum(checksum)
    }
}

/// Allow usage of crc64_t in unordered_set
///
impl Hash for Crc64 {
...
impl Crc64 {
    ...
    pub fn from_str(x: &'static str) -> Self {
        Self::from_bytes(x.as_bytes())
    }
    ...
    pub fn from_bytes(bytes: &[u8]) -> Self {
        ...
        for i in 0..bytes.len() {
            ...
            let idx = (accumulator ^ bytes[i] as u64) & 0xFF;

            accumulator = CRC64_TABLE[idx as usize] ^ (accumulator >> 8);
        }

        Crc64 {
            id_wrapper: accumulator,
        }
    }
