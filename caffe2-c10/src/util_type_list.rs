...
//-------------------------------------------[.cpp/pytorch/c10/util/TypeList.h]
lazy_static!{
    /*
    namespace c10 {
    namespace guts {
    ...
    struct false_higher_t : false_type {};
     ...
     * Example:
     *   tuple<int, string>  ==  to_tuple_t<typelist<int, string>>
     */
    template <class TypeList>
     ...
     * Example:
     *   typelist<int, string>  ==  from_tuple_t<tuple<int, string>>
     */
    template <class Tuple>
    struct from_tuple final {
      static_assert(
          false_t<Tuple>::value,
          "In typelist::from_tuple<T>, T must be tuple<...>.");
    };
    template <class... Types>
    struct from_tuple<tuple<Types...>> final {
     ...
     * Examples:
     *   typelist<int&, const string&&>  ==  filter_t<is_reference,
     ...
     * Examples:
     *   2  ==  count_if<is_reference, typelist<void, string, int&, bool, const
     * string&&, int>>
     */
    template <template <class> class Condition, class TypeList>
    ...
    struct contains {};
    template <class Type>
    struct contains<typelist<>, Type, void> : false_type {};
    template <class Type, class Head, class... Tail>
    struct contains<
        typelist<Head, Tail...>,
        Type,
        enable_if_t<is_same<Head, Type>::value>> : true_type {};
    template <class Type, class Head, class... Tail>
    struct contains<
        typelist<Head, Tail...>,
        Type,
        enable_if_t<!is_same<Head, Type>::value>>
        : contains<typelist<Tail...>, Type> {};
    } // namespace detail
    template <class TypeList, class Type>
    using contains = typename contains<TypeList, Type>::type;

    /**
     * Returns true iff the type trait is true for all types in the type list
     * Examples:
     *   true   ==  all<is_reference, typelist<int&, const float&&, const
     * MyClass&>>::value false  ==  all<is_reference, typelist<int&, const
     * float&&, MyClass>>::value
     */
    template <template <class> class Condition, class TypeList>
    struct all {
     ...
     * Examples:
     *   true   ==  true_for_any_type<is_reference, typelist<int, const
     * float&&, const MyClass>>::value false  ==
     * true_for_any_type<is_reference, typelist<int, const float,
     * MyClass>>::value
     */
    template <template <class> class Condition, class TypeList>
     ...
     * Example:
     *  typelist<int&, double&, string&>  ==  map_t<add_lvalue_reference_t,
    ...
    static_assert(
        is_same<int, last_t<typelist<double, float, int>>>::value,
        "");
    ...
    struct take_elements<TypeList, offset, index_sequence<Indices...>> final {
    ...
    struct take final {
      ...
      using type = typename 
          take_elements<TypeList, 0, make_index_sequence<num>>::type;
    };
    template <class TypeList, size_t num>
    ...
    struct drop final {
      ...
      using type = typename take_elements<
          TypeList,
          num,
          make_index_sequence<size<TypeList>::value - num>>::type;
    };
    template <class TypeList, size_t num>
     ...
     * condition. Example:
     *
     * 2 == find_if<typelist<char, int, char&, int&>, is_reference>::value
     */
     ...
     *     );
     *   //  sizes  ==  tuple<size_t, size_t, size_t>{8, 1, 4}
     *
     *   // Example 2
     *   auto shared_ptrs =
     ...
     *     );
     *   // shared_ptrs == tuple<shared_ptr<int>, shared_ptr<double>>()
     */
    namespace detail {
    template <class T>
    struct type_ final {
    ...
    struct map_types_to_values<typelist<Types...>> final {
      template <class Func>
      static tuple<result_of_t<Func(type_<Types>)>...> call(Func&& func) {
        return tuple<result_of_t<Func(type_<Types>)>...>{
            forward<Func>(func)(type_<Types>())...};
      }
    };
    } // namespace detail
    ...
    decltype(auto) map_types_to_values(Func&& func) {
      return map_types_to_values<TypeList>::call(forward<Func>(func));
    }

    } // namespace typelist
    } // namespace guts
    } // namespace c10
    //-------------------------------------------[.cpp/pytorch/c10/util/TypeList.cpp]
    */
}
