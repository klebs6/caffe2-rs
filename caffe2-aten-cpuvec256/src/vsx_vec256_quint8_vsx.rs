...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/cpu/vec/vec256/vsx/vec256_quint8_vsx.h]

pub const MASK_UNSIGNED: Vint16 = vec_splats(0xFF);

#[derive(Default)]
struct VectorizedQuint8Alt0 {
    vec0: Vuint8,
    vec1: Vuint8,
}

#[derive(Default)]
struct VectorizedQuint8Alt1 {
    vecb0: Vbool8,
    vecb1: Vbool8,
}

pub union VectorizedQuint8 {
    u0: VectorizedQuint8Alt0,
    u1: VectorizedQuint8Alt1,
}

pub mod vectorized_quint8 {
    ...
    pub type VecInternalType     = Vuint8;
    pub type VecInternalMaskType = Vbool8;

    lazy_static!{
        /*
        DEFINE_MEMBER_OP(operator==, quint8, vec_cmpeq)
          DEFINE_MEMBER_OP(operator!=, quint8, vec_cmpne)
          DEFINE_MEMBER_OP(operator<, quint8, vec_cmplt)
          DEFINE_MEMBER_OP(operator<=, quint8, vec_cmple)
          DEFINE_MEMBER_OP(operator>, quint8, vec_cmpgt)
          DEFINE_MEMBER_OP(operator>=, quint8, vec_cmpge)
          DEFINE_MEMBER_OP(operator+, quint8, vec_add)
          DEFINE_MEMBER_OP(operator-, quint8, vec_sub)
          DEFINE_MEMBER_OP(operator*, quint8, vec_mul)
          DEFINE_MEMBER_EMULATE_BINARY_OP(operator/, quint8, /)
          DEFINE_MEMBER_OP(maximum, quint8, vec_max)
          DEFINE_MEMBER_OP(minimum, quint8, vec_min)
          DEFINE_MEMBER_OP(operator&, quint8, vec_and)
          DEFINE_MEMBER_OP(operator|, quint8, vec_or)
          DEFINE_MEMBER_OP(operator^, quint8, vec_xor)
        */
    }
}
...
impl VectorizedQuint8 {
    ...
    #[inline(always)] pub fn new(val: &quint8) -> Self {
    
        todo!();
        /*
        : vec0(vec_splats(val.val_)),
        : vec1(vec_splats(val.val_)),
...
#[inline] pub fn maximum(
    a: &Vectorized<quint8>,
    b: &Vectorized<quint8>) -> Vectorized<quint8> {
