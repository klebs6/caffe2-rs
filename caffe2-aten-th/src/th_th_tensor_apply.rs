...
//-------------------------------------------[.cpp/pytorch/aten/src/TH/THTensorApply.h]
...
/**
  ...
  | the Tensor is contiguous, then A is equal
  | to the entire Tensor. Let's call the
  | inner tensor B. 2. We loop through the
  | indices in B, starting at its outermost
  ...
  | normally for a
  | 
  | Tensor. But because we are guaranteed
  | the subsequent data is contiguous in
  | memory, we can simply loop for sizeof(A)
  | iterations and perform the operation,
  | without having to follow the order described
  | by the strides of A. 3. As an optimization,
  | we merge dimensions of A that are contiguous
  | in memory. For example, if A is a 3x3x3x3
  | tensor narrowed from a 3x3x4x3 tensor,
  | then the first two dimensions can be
  | merged for the purposes of APPLY, reducing
  | the number of nested loops.
  |
  */
#[macro_export] macro_rules! __TH_TENSOR_APPLYX_PREAMBLE {
    ($TYPE:ident, $TENSOR:ident, $DIM:ident, $ALLOW_CONTIGUOUS:ident) => {
        /*
        
          TYPE *TENSOR##_data = NULL; 
          i64 *TENSOR##_counter = NULL, *TENSOR##_sizes = NULL, *TENSOR##_strides = NULL, *TENSOR##_dimOffset = NULL; 
          i64 TENSOR##_stride = 0, TENSOR##_size = 0, TENSOR##_dim = 0, TENSOR##_i, TENSOR##_n; 
          ...
          { 
            ...
            if (!TENSOR##_contiguous) { 
              ...
              TENSOR##_counter = (i64*)THAlloc(sizeof(i64)*(3*TENSOR##_dim)); 
              ...
              TENSOR##_dim = 1;
              TENSOR##_counter = (i64*)THAlloc(sizeof(i64)*3);
...
#[macro_export] macro_rules! __TH_TENSOR_APPLYX_UPDATE_COUNTERS {
    ($TENSOR:ident, $ALWAYS_UPDATE:ident) => {
        /*
...
#[macro_export] macro_rules! th_tensor_apply3_d {
    ($TYPE1:ident, $TENSOR1:ident, $TYPE2:ident, $TENSOR2:ident, $TYPE3:ident, $TENSOR3:ident, $DIM:ident, $CODE:ident) => {
        /*
...
#[macro_export] macro_rules! th_tensor_apply3 {
...
#[macro_export] macro_rules! th_tensor_apply2_d {
    ($TYPE1:ident, $TENSOR1:ident, $TYPE2:ident, $TENSOR2:ident, $DIM:ident, $CODE:ident) => {
        /*
...
#[macro_export] macro_rules! th_tensor_apply2 {
...
#[macro_export] macro_rules! th_tensor_apply_d {
    ($TYPE:ident, $TENSOR:ident, $DIM:ident, $CODE:ident) => {
        /*
...
#[macro_export] macro_rules! th_tensor_apply {
...
/**
  | Calcuate the memory offset of an element
  | in a tensor. The strategy is below: 1.
  | convert the line index(the index of
  | the element) to the indexs(coordinates)
  | in the tensor.
  | 
  | It can hinted by a classical problem:
  | Getting each individual digit from
  | a whole integer(Decimal base).
  | 
  | A N-digit decimal base number could
  | be view as a N-dimension tensor and the
  | sizes of the tensor are 10.
  | 
  | So the value the whole integer is the
  | line index. And the digits could be viewed
  | as the indexes in different dimensions.
  | 2. convert the indexs(coordinates)
  | in the tensor to the memory offset.
  | 
  | You can get the detailes in the for-statement
  | iterations.
  | 
  | The macro is only used in the first element
  | in each thread. For the rest, the memory
  | offset could update according to info
  | of the tensor in order to get better performance.
  | So we should also record the each indexs
  | in coresponding dimension of first
  | element.
  | 
  | The recorded info is stored in the TENSOR##_counter_tmp.
  |
  */
#[macro_export] macro_rules! __TH_TENSOR_APPLYX_CAL_MEMORY_OFFSET {
    ($TENSOR:ident) => {
        /*
        
          i64 *TENSOR##_counter_tmp = (i64*)THAlloc(sizeof(i64) * TENSOR##_dim);                 
...
/**
  | The macro update the indexes in each
  | dimension of the elements except for
  | the first one allocated in each thread.
  | 
  | For a tensor, if the index of some dimension
  | reaches the size of the corresponding
  | dimension. It will carry and clear.
  | 
  | If the index of next high dimension does
  | do, the index of next high dimension
  | should carry and clear, too.
  | 
  | The momery offset calculatation is
  | a little confusing. If current index
  | carries, the current index is set to
  | 0. So the offset should decrease by size*stride
  | of the last dimension. Then the index
  | next high dimension increases by 1.
  | So the offset should increase by stride
  | of next high dimension.
  |
  */
#[macro_export] macro_rules! __TH_TENSOR_APPLYX_UPDATE_COUNTERS_PARALLEL {
    ($TENSOR:ident) => {
        /*
