...
//-------------------------------------------[.cpp/pytorch/aten/src/TH/THTensorDimApply.h]

/**
  | This is an example of SIZE_CHECK argument
  | passable to TH_TENSOR_DIM_APPLY3.
  |
  | The TENSOR1, TENSOR2, TENSOR3, DIMENSION will
  | be expanded the same way as
  | TH_TENSOR_DIM_APPLY3.
  |
  | Specifically, this check ensures that TENSOR1,
  | TENSOR2, TENSOR3 have same size except for
  | DIMENSION.
  |
  */
#[macro_export] macro_rules! th_tensor_dim_apply3_size_eq_except_dim {
...
#[macro_export] macro_rules! th_tensor_dim_apply3 {
    ($TYPE1:ident, $TENSOR1:ident, $TYPE2:ident, $TENSOR2:ident, $TYPE3:ident, $TENSOR3:ident, $DIMENSION:ident, $SIZE_CHECK:ident, $CODE:ident) => {
        /*
        
        { 
          TYPE1 *TENSOR1##_data = NULL; 
          TH_UNUSED i64 TENSOR1##_stride = 0, TENSOR1##_size = 0; 
          TYPE2 *TENSOR2##_data = NULL; 
          TH_UNUSED i64 TENSOR2##_stride = 0, TENSOR2##_size = 0; 
          TYPE3 *TENSOR3##_data = NULL; 
          TH_UNUSED i64 TENSOR3##_stride = 0, TENSOR3##_size = 0; 
          i64 *TH_TENSOR_DIM_APPLY_counter = NULL; 
          ...
          TH_TENSOR_DIM_APPLY_counter = (i64*)THAlloc(sizeof(i64)*(THTensor_nDimensionLegacyNoScalars(TENSOR1))); 
          for(TH_TENSOR_DIM_APPLY_i = 0; TH_TENSOR_DIM_APPLY_i < THTensor_nDimensionLegacyNoScalars(TENSOR1); TH_TENSOR_DIM_APPLY_i++) 
...
/**
 | Similar to DIM_APPLY(...) but we maintain two
 | sets of pointers: one for the first tensor and
 | one for the second. 
 |
 | The two tensors must have the same shape, other
 | than at the specified DIMENSION. 
 |
 | This function makes it easy to store the output
 | from reducing the TENSOR at index. 
 |
 | For example, in the sum example described
 | below, we could instead do:
 |
 | i64 i = 0;
 | TYPE1 sum;
 |
 ...
 |
 | In particular, we guarantee that the offset
 | into TENSOR2 will be what you would get if you
 | applied all of the index values used to
 | generate the offset into TENSOR1.
 |
 */
#[macro_export] macro_rules! th_tensor_dim_apply2 {
    ($TYPE1:ident, $TENSOR1:ident, $TYPE2:ident, $TENSOR2:ident, $DIMENSION:ident, $CODE:ident) => {
        /*
...
/**
 | The basic idea for DIM_APPLY: Given a TENSOR
 | and a DIMENSION, provide access to the data
 | stored at all sets of dimension values other
 | than DIMENSION, such that we can get all the
 | values at those fixed indices for the various
 | values at DIMENSION.
 |
 | Suppose we have a 2x3x4 Tensor A, and we have
 | DIMENSION=2. Then we will hit CODE (2x3) times,
 | and the pointer into storage will be at:
 |
 | A[0][0]
 | A[0][1]
 | A[0][2]
 ...
 | A[1][2]
 |
 | And at each point, we can access the data for
 | each of the four elements of the Tensor via
 | TENSOR##_stride. So for example, if we wanted
 | to sum the elements there, we could do:
 |
 | i64 i = 0;
 | TYPE sum;
 ...
 | }
 |
 | Note that we don't have to have DIMENSION be
 | the last tensor. If we have DIMENSION=1, then
 | we will hit the code (2x4) times, with pointer
 | into the storage at:
 |
 | offset +
 ...
 |
 | So we can again sum over the values at
 | DIMENSION with the other indices fixed.
 |
 */
#[macro_export] macro_rules! th_tensor_dim_apply {
    ($TYPE:ident, $TENSOR:ident, $DIMENSION:ident, $CODE:ident) => {
        /*
        
        { 
          TYPE *TENSOR##_data = NULL; 
          i64 TENSOR##_stride = 0, TENSOR##_size = 0; 
          i64 *TH_TENSOR_DIM_APPLY_counter = NULL; 
          ...
          TH_TENSOR_DIM_APPLY_counter = (i64*)THAlloc(sizeof(i64)*(THTensor_nDimensionLegacyAll(TENSOR))); 
