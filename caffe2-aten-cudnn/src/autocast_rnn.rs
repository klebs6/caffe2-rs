/*!
  | Autocast wrapper for CuDNN RNNs (the
  | weight reflattening needs special
  | attention)
  |
  */
crate::ix!();

//-------------------------------------------[.cpp/pytorch/aten/src/ATen/cudnn/AutocastRNN.cpp]

/**
  | To be registered for the "_cudnn_rnn(...)"
  |    schema.
  |
  | _cudnn_rnn is autograd-exposed
  | (test_autocast_cudnn_rnn in test_cuda.py
  | includes a test to confirm)
  */
pub fn autocast_cudnn_rnn_cast_reflatten(
    input:          &Tensor,
    weight:         &[Tensor],
    weight_stride0: i64,
    weight_buf_opt: &Option<Tensor>,
    hx:             &Tensor,
    cx:             &Option<Tensor>,
    mode:           i64,
    hidden_size:    i64,
    ...
    bidirectional:  bool,
    batch_sizes:    &[i32],
    dropout_state:  &Option<Tensor>) -> (Tensor,Tensor,Tensor,Tensor,Tensor) {
    ...
    todo!();
      ...
      return at::_cudnn_rnn(
          ...
          needs_cast_and_flatten ? &[Tensor](redispatch_weight) : weight,
          weight_stride0,
