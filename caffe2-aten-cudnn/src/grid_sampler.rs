...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/cudnn/GridSampler.cpp]

// See Note [ATen preprocessor philosophy]
#[cfg(not(AT_CUDNN_ENABLED))]
pub fn cudnn_grid_sampler_forward(
        input_t: &Tensor,
        grid_t:  &Tensor) -> Tensor {
...
pub fn cudnn_grid_sampler_backward(
        input_t:       &Tensor,
        grid_t:        &Tensor,
        grad_output_t: &Tensor) -> (Tensor,Tensor) {
...
pub fn cudnn_grid_sampler_forward(
    ...
    todo!();
      ...
      auto output_t = empty({0}, input->options());
...
/**
  | NB: CuDNN does not support output mask;
  | you always get both gradients.
  |
  */
#[cfg(AT_CUDNN_ENABLED)]
pub fn cudnn_grid_sampler_backward(
    ...
    todo!();
      ...
      auto grad_input_t = empty({0}, input->options());
      grad_input_t.resize_(input->sizes());
      auto grad_grid_t = empty({0}, grid->options());
      ...
      return tuple<Tensor, Tensor>{ grad_input_t, grad_grid_t };
        */
}
