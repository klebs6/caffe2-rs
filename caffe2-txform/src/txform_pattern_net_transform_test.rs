...
use crate::OperatorStorage;

lazy_static!{
    static ref counter: AtomicI32 = AtomicI32::new(0);
}

pub struct DummyCounterOp {
...
num_inputs!{DummyCounterOp1, (0,INT_MAX)}

num_outputs!{DummyCounterOp1, (0,INT_MAX)}

allow_inplace!{DummyCounterOp1, vec![(0, 0), (1, 1)]}
...
num_inputs!{DummyCounterOp2, (0,INT_MAX)}

num_outputs!{DummyCounterOp2, (0,INT_MAX)}

allow_inplace!{DummyCounterOp2, vec![(0, 0), (1, 1)]}
...
num_inputs!{DummyCounterOp3, (0,INT_MAX)}

num_outputs!{DummyCounterOp3, (0,INT_MAX)}

allow_inplace!{DummyCounterOp3, vec![(0, 0), (1, 1)]}
...
/**
 ...
 |                |--(Op2)--|
 |
 | In this test, the two "parallel" modules have
 | intersecting execution orders.
 |
 | We wish to test that the pattern match can
 | still detect the two modules, separately.
 |
 | Furthermore, we will apply the transform to G,
 | TWICE.
 |
...
/**
 ...
 | G = P -> P
 |
 | In this test, we fuse a subgraph with two
 | inputs and two outputs, into one operator.
 |
 | This will ensure that we can allow a single
 | edge to represent multiple blob names (the input
 | and output of R are both 2 blobs).
 |
 | This will also ensure that patternmatch can
 | traverse "backwards", from a node to its parent.
 |
 | Furthermore, this tests for repeat matches,
 | since matching on either of the first two Op1
 | nodes will produce a match, but they are
 | identical.
 |
