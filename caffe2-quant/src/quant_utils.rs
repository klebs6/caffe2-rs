...
#[inline] pub fn check_and_saturate<T>(max_val: T, element: *mut T) -> bool {
...
/**
  ...
  | 
  | The meaning of these values is as the
  | constants in the quantization equation
  | 
  | real_value = scale * (quantized_value - zero_point)
  | 
  | In other words, 'zero_point' is the
  | quantized value that corresponds to
  | the real value 0, and 'scale' is the difference
  | of real values corresponding to consecutive
  | quantized values.
  |
  */
...
#[inline] pub fn choose_quantization_params(
    ...
    force_scale_power_of_two: Option<bool>,
    reduce_range:             Option<bool>) -> TensorQuantizationParams {
...
/**
  ...
  |
  */
pub const kConv1dSqueezeDim: i64 = 0;

#[inline] pub fn make_arg_for_conv1d(arg: &List<i64>, base_value: i64) -> List<i64> {
...
/**
  | The range for using FP16 quantization
  | of weights requires that the elements
  | should be in the range of [5.96e-8, 65504].
  | 
  | If it is out of range, then the number
  | will be saturated to max or min representable
  | values by FP16.
  |
  */
#[inline] pub fn handle_weights_saturation(n: i64, weight: *mut f32)  {
