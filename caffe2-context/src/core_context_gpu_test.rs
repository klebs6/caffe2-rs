...
#[inline] pub fn get_stream_for_handle(handle: CublasHandle) -> CudaStream {
...
/**
  | A test function to return a stream address
  | from a temp CUDA context. You should not use
  | that stream though, because the actual stream
  | is destroyed after thread exit.
  */
#[inline] pub fn test_get_stream_address(ptr: *mut CudaStream)  {
