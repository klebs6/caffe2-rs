...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/UnfoldBackward.h]

pub type UnfoldBackwardFn = fn(
        grad_in: &mut Tensor,
        grad:    &Tensor,
        dim:     i64,
        size:    i64,
...
/**
  ...
  |
  | grad_in does not mean that it is a gradient wrt
  | to input,
  |
  | grad_in/grad_out is just an input/output of
  | unfold_backward kernel.
  |
  */
pub fn make_unfold_backward_iter_over_grad_out(
    grad_out: &mut Tensor,
    grad_in:  &Tensor,
    dim:      i64,
    size:     i64,
    step:     i64) -> TensorIterator {
    ...
    todo!();
      ...
      auto idx_dim = arange(
        0, iter_dim_size, grad_in.options().dtype(kLong)
      );

      auto grad_out_dim = ensure_nonempty_dim(grad_out.dim());

      auto idx_dim_strides = vector<i64>(grad_out_dim, 0);
      auto idx_dim_sizes = vector<i64>(grad_out_dim, 1);
...
pub fn make_unfold_backward_iter_over_grad_in(
    grad_out: &mut Tensor,
    grad_in:  &Tensor,
    dim:      i64,
    size:     i64,
    ...
    todo!();
      ...
      auto idx_dim = arange(
        0, grad_in_dim_size, grad_in.options().dtype(kLong)
      );

      auto idx_dim_strides = vector<i64>(grad_in_dim, 0);
      auto idx_dim_sizes = vector<i64>(grad_in_dim, 1);
      ...
      auto idx_last_dim = arange(
        0, grad_in_last_dim_size, grad_in.options().dtype(kLong)
      );

      auto idx_last_dim_strides = vector<i64>(grad_in_dim, 0);
      auto idx_last_dim_sizes = vector<i64>(grad_in_dim, 1);
...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/UnfoldBackward.cpp]
