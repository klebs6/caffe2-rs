// # vim: ft=none
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/metal/MetalShaders.h]

pub const PT_METAL_SHADERS: &'static str = r#"
...
kernel void reshape(texture2d_array<half, access::read> in_arr[[texture(0), function_constant(reshape_in_is_arr)]],
    ...
    const usize numel1 = H1 * W1 * C1 * N1;
    ...
    for (int idx = 0; idx < 4; ++idx){
        ...
        usize linear_idx = n2 * C2 * H2 * W2 + offset * H2 * W2 + gid.y * W2 + gid.x;
...
kernel void transpose(texture2d_array<half, access::read>in_arr[[texture(0),function_constant(transpose_in_is_arr)]],
    ...
    for (int idx = 0; idx < 4; ++idx){
        ushort offset = 4 * s2 + idx;
        usize linear_idx2 = n2 * C2 * H2 * W2 + offset * H2 * W2 + gid.y * W2 + gid.x;
