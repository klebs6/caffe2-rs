...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/quantized/cpu/qadd.cpp]
...
/**
  ...
  |
  */
pub fn add_out<const ReLUFused: bool = false>(
        out:   &mut Tensor,
        self_: &Tensor,
        other: &Tensor) -> Tensor {
...
pub fn add_scalar_out<const ReLUFused: bool = false>(
        out:   &mut Tensor,
        self_: &Tensor,
        other: &Scalar) -> Tensor {
    ...
    todo!();
      ...
      //   z' = q_min
      //   Xq' = requantize_from_int(Xq - z + c_q, s/s', z')
      // If q_max < z - c_q
      ...
      AT_DISPATCH_QINT_TYPES(self.scalar_type(), "qadd_scalar", [&]() {
        ...
        if (q_min > z - c_q) {
          ...
          set_quantizer_(out, make_per_tensor_affine_quantizer(
              s_prime, z_prime, self.scalar_type()));
          if (ReLUFused) {
            native::relu_quantized_cpu_(out);
          }
        }
      });
      return out;
...
pub fn qnnpack_add<const ReLUFused: bool = false>(
        qa:         Tensor,
        qb:         Tensor,
        scale:      f64,
    ...
    todo!();
      ...
      usize num_elems = qa_contig.numel() / qa_contig.size(0);
      ...
      const pytorch_qnnp_status createStatus = pytorch_qnnp_create_add_nc_q8(
          ...
          b_scale /* b scale */,
          static_cast<u8>(zero_point) /* sum zero_point */,
          scale /* sum scale */,
      ...
      const pytorch_qnnp_status setupStatus = pytorch_qnnp_setup_add_nc_q8(
          ...
          (u8*)qa_contig.data_ptr<quint8>() /* a data */,
          num_elems /* A stride */,
          (u8*)qb_contig.data_ptr<quint8>() /* b data */,
          num_elems /* B stride */,
...
pub fn qadd<const ReLUFused: bool = false>(
        qa:         Tensor,
        qb:         Tensor,
        scale:      f64,
        zero_point: i64) -> Tensor {
...
pub fn qadd_out<const ReLUFused: bool = false>(
        qa:  Tensor,
        qb:  Tensor,
        out: Tensor) -> Tensor {
...
pub fn qadd_scalar<const ReLUFused: bool = false>(
        qa: Tensor,
        b:  &Scalar) -> Tensor {
...
pub fn qadd_scalar2<const ReLUFused: bool = false>(
        b:  Scalar,
        qa: Tensor) -> Tensor {
...
pub fn qadd_scalar_out<const ReLUFused: bool = false>(
        qa:  Tensor,
        b:   &Scalar,
        out: Tensor) -> Tensor {
...
/**
  ...
  | `quantized::add` is merged into
  | `quantized::add`
  |
  */
pub fn qadd_scalar_tensor<const ReLUFused: bool = false>(
        qa: Tensor,
        b:  Tensor) -> Tensor {
...
/**
  ...
  |
  */
pub fn qadd_scalar_tensor_out<const ReLUFused: bool = false>(
        qa:  Tensor,
        b:   Tensor,
        out: Tensor) -> Tensor {
