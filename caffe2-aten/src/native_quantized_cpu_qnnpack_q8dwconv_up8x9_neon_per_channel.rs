...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/quantized/cpu/qnnpack/src/q8dwconv/up8x9-neon-per-channel.c]

pub fn pytorch_q8dwconv_ukernel_up8x9_per_channel_neon(
        channels:            usize,
        output_width:        usize,
    ...
    todo!();
      ...
      if (input_stride == 3 * sizeof(void*)) {
        for (; output_width >= 3; output_width -= 3) {
          ...
          const u8* i10 = input[1];
          const u8* i20 = input[2];
          const u8* i01 = input[3];
          const u8* i11 = input[4];
          const u8* i21 = input[5];
          const u8* i02 = input[6];
          const u8* i12 = input[7];
          const u8* i22 = input[8];
          const u8* i03 = input[9];
          const u8* i13 = input[10];
          const u8* i23 = input[11];
          const u8* i04 = input[12];
          const u8* i14 = input[13];
          const u8* i24 = input[14];

          u8* output0 = output;
          u8* output1 = output0 + channels + output_increment;
          u8* output2 = output1 + channels + output_increment;
          ...
          if (c != 0) {
            const usize c_predecrement = 8 - c;
          ...
          output = (u8*)((uintptr_t)output2 + output_increment);
        }
        if (output_width == 0) {
          return;
        }
