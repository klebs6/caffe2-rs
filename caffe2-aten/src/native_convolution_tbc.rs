...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/ConvolutionTBC.cpp]
...
pub fn conv_tbc(
    ...
    todo!();
      ...
      for (int k = 0; k < kw; k++) {
        int iShift = max(0, static_cast<int>(k - real_pad));
        int oShift = max(0, static_cast<int>(real_pad - k));
        int t = min(ilen + real_pad - k, olen) - oShift;
...
pub fn conv_tbc_backward(
    ...
    todo!();
      ...
      Tensor dInput = zeros_like(input, LEGACY_CONTIGUOUS_MEMORY_FORMAT);
      for (int k = 0; k < kw; k++) {
        int iShift = max(0, k - real_pad);
        int oShift = max(0, real_pad - k);
        int t = min(ilen + real_pad - k, olen) - oShift;
      ...
      Tensor dWeight = zeros_like(weight, LEGACY_CONTIGUOUS_MEMORY_FORMAT);
      for (int k = 0; k < kw; k++) {
        int iShift = max(0, k - real_pad);
        int oShift = max(0, real_pad - k);
        int t = min(ilen + real_pad - k, olen) - oShift;
      ...
      Tensor dBias = zeros_like(bias, LEGACY_CONTIGUOUS_MEMORY_FORMAT);
      auto tmp = dOutput.sum(0, false);
      dBias.copy_(tmp.sum(0));

      return make_tuple(dInput, dWeight, dBias);
        */
}
