...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/cpu/StackKernel.h]

lazy_static!{
    /*
    using stack_serial_fn = void(*)(Tensor &, TensorList, i64);
    */
}

declare_dispatch!{stack_serial_fn, stack_serial_stub}

//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/cpu/StackKernel.cpp]
...
pub fn stack_serial_kernel_impl<Scalar>(
        result:  &mut Tensor,
        tensors: TensorList,
        dim:     i64)  {
    ...
    todo!();
        /*
          ...
          result.scalar_type(), "stack_serial_kernel", [&]() {
            stack_serial_kernel_impl<Scalar>(result, tensors, dim);
          });
        */
}
