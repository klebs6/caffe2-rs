...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/InferSize.h]

/**
  | Infers the size of a dim with size -1, if it
  | exists. Also checks that new shape is
  | compatible with the number of elements.
  |
  | templated to handle std::vector<i64> and
  | DimVector use cases, see below
  |
  */
#[inline] pub fn infer_size_impl<ResultVec>(
        shape: &[i32],
        numel: i64,
        res:   &mut ResultVec)  {
    ...
    todo!();
        /*
            i64 newsize = 1;
      auto infer_dim = c10::optional<i64>();
      for (i64 dim = 0, ndim = shape.size(); dim != ndim; dim++) {
        if (shape[dim] == -1) {
          if (infer_dim) {
