// # vim: ft=none
crate::ix!();

//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/quantized/cpu/qreduction.cpp]

#[cfg(target_feature = "qnnpack")]
pub fn qnnpack_mean(
    input:   &Tensor,
    dim:     &[i32],
    keepdim: bool) -> Tensor {

    todo!();
        /*
            Tensor output;
      TORCH_CHECK(
          input.ndimension() == 4,
          "qnnpack_global_average_pool: Expected input to be 4-dimensional: got ",
          input.ndimension());
      TORCH_CHECK(
          dim.size() == 2,
          "qnnpack_global_average_pool: dim size must be a tuple of two ints");
      TORCH_CHECK(
          dim[0] == 2 && dim[1] == 3,
          "qnnpack_global_average_pool: Reduction dimensions must match last 2 dimensions of input tensor")

      const i64 batch_size = input.size(0);
      const i64 inC = input.size(1);
      const i64 inH = input.size(2);
      const i64 inW = input.size(3);

      Tensor input_contig = input.contiguous(MemoryFormat::ChannelsLast);

      initQNNPACK();
      const auto scale = input_contig.q_scale();
      const auto zero_point = input_contig.q_zero_point();
      const auto outC = inC;

      output = _empty_affine_quantized(
          keepdim ? IntArrayRef{batch_size, outC, 1, 1}
                  : IntArrayRef{batch_size, outC},
          device(kCPU).dtype(kQUInt8),
          scale,
          zero_point);

      pytorch_qnnp_operator_t qnnpack_operator{nullptr};
      const pytorch_qnnp_status createStatus =
          pytorch_qnnp_create_global_average_pooling_nwc_q8(
              inC,
              zero_point,
              scale,
              zero_point,
              scale,
              u8::min /* output min */,
              u8::max /* output max */,
              0,
              &qnnpack_operator);

      CAFFE_ENFORCE(
          createStatus == pytorch_qnnp_status_success,
          "failed to create QNNPACK Global Average Pooling operator");
      unique_ptr<pytorch_qnnp_operator, QnnpackOperatorDeleter>
          qnnpack_uniq_ptr(qnnpack_operator);

      const pytorch_qnnp_status setupStatus =
          pytorch_qnnp_setup_global_average_pooling_nwc_q8(
              qnnpack_operator,
              batch_size,
              inH * inW,
              (u8*)input_contig.data_ptr<quint8>() /* input data */,
              inC,
              (u8*)output.data_ptr<quint8>() /* output data */,
              outC);
      CAFFE_ENFORCE(
          setupStatus == pytorch_qnnp_status_success,
          "failed to setup QNNPACK Global Average Pooling operator");
      pthreadpool_t threadpool = pthreadpool_();
      const pytorch_qnnp_status runStatus =
          pytorch_qnnp_run_operator(qnnpack_operator, threadpool);
      TORCH_INTERNAL_ASSERT(
          runStatus == pytorch_qnnp_status_success,
          "failed to run QNNPACK Global Average Pool operator");
      return output;
        */
}

pub fn mean_out_quantized_cpu(
        self_:     &Tensor,
        dim:       &[i32],
        keepdim:   bool,
        opt_dtype: Option<ScalarType>,
        result:    &mut Tensor) -> &mut Tensor {
    
    todo!();
        /*
            #ifdef USE_PYTORCH_QNNPACK
      if (globalContext().qEngine() == QEngine::QNNPACK &&
          self.scalar_type() == kQUInt8 &&
          // QNNPACK currently is only supported for NCHW + dim=(2, 3)
          // Remove these checks after generic version is implemented.
          self.ndimension() == 4 && dim.size() == 2 && dim[0] == 2 && dim[1] == 3) {
        result = qnnpack_mean(self, dim, keepdim);
        return result;
      }
    #endif
      auto self_dequantized = self.dequantize();
      auto result_dequantized =
          native::mean_cpu_gpu(self_dequantized, dim, keepdim, opt_dtype);
      result = quantize_per_tensor(
          result_dequantized,
          self.q_scale(),
          self.q_zero_point(),
          opt_dtype.value_or(self.scalar_type()));
      return result;
        */
}

pub fn mean_quantized_cpu_a(
        self_: &Tensor,
        dtype: Option<ScalarType>) -> Tensor {
    
    todo!();
        /*
            Tensor result;
      mean_out_quantized_cpu(self, IntArrayRef{}, false, dtype, result);
      return result;
        */
}

pub fn mean_quantized_cpu_b(
        self_:   &Tensor,
        dim:     &[i32],
        keepdim: bool,
        dtype:   Option<ScalarType>) -> Tensor {
    
    todo!();
        /*
            Tensor result;
      mean_out_quantized_cpu(self, dim, keepdim, dtype, result);
      return result;
        */
}

pub fn mean_quantized_cpu_c(
        self_:   &Tensor,
        dim:     DimnameList,
        keepdim: bool,
        dtype:   Option<ScalarType>) -> Tensor {
    
    todo!();
        /*
            return mean_quantized_cpu(
          self, dimnames_to_positions(self, dim), keepdim, dtype);
        */
}

pub fn mean_out_quantized_cpu_mut(
        result:    &mut Tensor,
        self_:     &Tensor,
        dim:       DimnameList,
        keepdim:   bool,
        opt_dtype: Option<ScalarType>) -> &mut Tensor {
    
    todo!();
        /*
            return mean_out_quantized_cpu(
          self, dimnames_to_positions(self, dim), keepdim, opt_dtype, result);
        */
}
