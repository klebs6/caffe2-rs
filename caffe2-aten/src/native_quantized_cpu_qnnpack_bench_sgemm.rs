...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/quantized/cpu/qnnpack/bench/sgemm.cc]
...
pub fn sgemm_benchmark(
        state: &mut BenchmarkState,
        sgemm: PyTorchSGemmUKernelFunction,
        mc:    u32,
        nc:    u32,
        kc:    u32,
        mr:    u32,
    ...
    todo!();
        /*
            const usize ncStride = roundUp(nc, np);
      const usize kcStride = roundUp(kc, kr);

      random_device randomDevice;
      auto rng = mt19937(randomDevice());
      auto f32rng = bind(uniform_real_distribution<float>(), rng);

      vector<float> a(mc * kc);
      generate(a.begin(), a.end(), ref(f32rng));
      vector<float> k(nc * kc);
      generate(k.begin(), k.end(), ref(f32rng));
      vector<float> b(nc);
      generate(b.begin(), b.end(), ref(f32rng));
      vector<float, AlignedAllocator<float, 32>> w(
          ncStride * kcStride + ncStride);
      fill(w.begin(), w.end(), 0.0f);
      pytorch_pack_sgemm_w(nc, kc, nr, kr, k.data(), b.data(), w.data());
      vector<float> c(mc * nc);
      ...
      for (auto _ : state) {
        for (u32 m = 0; m < mc; m += mr) {
          const u32 mb = min(mc - m, mr);
          for (u32 n = 0; n < nc; n += nr) {
            const u32 nb = min(nc - n, nr);
            sgemm(
                mb,
                nb,
                kc,
      ...
      state.SetItemsProcessed(u64(state.iterations()) * 2 * mc * nc * kc);
        */
}
...
pub fn sgemm(
    ...
    todo!();
        /*
            const usize mc = state.range(0);
      const usize nc = state.range(1);
      const usize kc = state.range(2);
