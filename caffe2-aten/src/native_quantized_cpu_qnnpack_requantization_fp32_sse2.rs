...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/quantized/cpu/qnnpack/src/requantization/fp32-sse2.c]

pub fn pytorch_qnnp_requantize_fp32_sse2(
    n:          Size,
    input:      *const i32,
    scale:      f32,
    zero_point: u8,
    ...
    todo!();
      ...
      for (; n != 0; n -= 16) {
         ...
         * MXCSR rounding mode):
         * - Large i32 values can't be exactly represented as FP32. CVTDQ2PS
        ...
        /*
         * Convert scaled FP32 result to i32 using CVTPS2DQ instruction from x86
         ...
         * e.g. i32(float(INT32_MAX)) == INT32_MIN! This happens because
         * float(INT32_MAX) rounds to 2**31, which overflows i32 when it is
         ...
         * 2147483520 (compare to INT32_MAX = 2147483647), which fits into i32
         * without overflow.
         */
