/*!
  | this convertor will:
  |
  | 1) take a Tensor object and wrap it in the
  | DLPack tensor
  |
  | 2) take a dlpack tensor and convert it to the
  | ATen Tensor
  |
  */
...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/DLConvertor.h]
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/DLConvertor.cpp]

pub fn get_dld_ata_type(t: &Tensor) -> DLDataType {
...
pub fn get_dl_device(
        tensor:    &Tensor,
        device_id: &i64) -> DLDevice {
...
pub fn get_aten_device(ctx: &DLDevice) -> Device {
...
pub fn to_scalar_type(dtype: &DLDataType) -> ScalarType {
...
//
pub struct ATenDLMTensor {
    handle: Tensor,
    tensor: DLManagedTensor,
}

pub fn deleter(arg: *mut DLManagedTensor)  {
...
/**
  | This function returns a shared_ptr to memory
  | managed DLpack tensor constructed out of ATen
  | tensor
  |
  */
pub fn to_dlp_ack(src: &Tensor) -> *mut DLManagedTensor {
    ...
    todo!();
      ...
      atDLMTensor->tensor.dl_tensor.shape =
          const_cast<i64*>(src.sizes().data());
      atDLMTensor->tensor.dl_tensor.strides =
          const_cast<i64*>(src.strides().data());
...
pub fn from_dlp_ack(src: *const DLManagedTensor) -> Tensor {
