...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/quantized/cpu/qnnpack/src/q8conv/8x8-neon.c]

pub fn pytorch_q8conv_ukernel_8x8_neon(
        mr:                   usize,
        nr:                   usize,
        kc:                   usize,
        ks:                   usize,
    ...
    todo!();
      ...
      do {
        const u8* restrict a0 = *a++;
        const u8* restrict a1 = *a++;
        const u8* restrict a2 = *a++;
        const u8* restrict a3 = *a++;
        const u8* restrict a4 = *a++;
        const u8* restrict a5 = *a++;
        const u8* restrict a6 = *a++;
        const u8* restrict a7 = *a++;
