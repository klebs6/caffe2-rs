...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/quantized/cpu/qnnpack/src/q8gemm_sparse/8x4c1x4-dq-packedA-sse2.c]

#[macro_export] macro_rules! convert_to_fp_and_transpose {
    ($a:ident, $b:ident, $c:ident, $d:ident, $t_a:ident, $t_b:ident, $t_c:ident, $t_d:ident) => {
        /*
...
pub fn pytorch_q8gemm_dq_sparse_1x4_ukernel_8x4_packeda_sse2(
        mr:                   Size,
        nr:                   Size,
        a_packed:             *const u8,
        packed_w:             *const u8,
        ...
        c:                    *mut f32,
        c_stride:             Size,
        output_channel_index: Size,
        quantization_params:  [PyTorchQnnpConvDynamicQuantizationParams; 1])  {
    ...
    todo!();
      ...
      for (i32 n = 0; n < nr; n++) {
        ...
        const i16 b_zero_point =
          (i16)(u16)quantization_params->kernel_zero_points[
          output_channel_index + n];

        i32 num_blocks = w_row_ptr[n+1] - w_row_ptr[n];
        ...
        const u8* temp_packed_w = packed_w + w_row_ptr[n] * COL_BLOCK_SIZE;
        ...
        const u32* temp_w_block_ids_ptr = w_block_ids_ptr + w_row_ptr[n];
        while (num_blocks > 1) {
          // Load two 1x4 uint8 blocks 2 ints
          const u8* b_ptr = temp_packed_w;
          ...
          const i16 b_0 = (i16)((u16)(b_ptr[0]));
          const i16 b_1 = (i16)((u16)(b_ptr[1]));
          const i16 b_2 = (i16)((u16)(b_ptr[2]));
          const i16 b_3 = (i16)((u16)(b_ptr[3]));
          const i16 b_4 = (i16)((u16)(b_ptr[4]));
          const i16 b_5 = (i16)((u16)(b_ptr[5]));
          const i16 b_6 = (i16)((u16)(b_ptr[6]));
          const i16 b_7 = (i16)((u16)(b_ptr[7]));
          ...
          i32 col_block_id_0 = temp_w_block_ids_ptr[0];
          // Load column id of the second 1x4 block
          i32 col_block_id_1 = temp_w_block_ids_ptr[1];
          const __m128i va0 =
