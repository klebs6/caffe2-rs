...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/cpu/UnfoldBackwardKernel.cpp]
...
/**
  ...
  */
pub fn unfold_backward_internal_kernel<Scalar>(
    iter:                    &mut TensorIterator,
    size:                    i64,
    step:                    i64,
    grad_in_dim_stride:      i64,
    grad_in_last_dim_stride: i64,
    grad_in_dim_size:        i64,
    grad_out_dim_stride:     i64,
    is_step_ge_size:         bool)  {
      ...
      auto loop = [&](char** data, const i64* strides, i64 nelems) {
        ...
        if (is_step_ge_size) {
          ...
          for (i64 elem = 0; elem < nelems; ++elem) {
            auto* RESTRICT grad_out_data = reinterpret_cast<Scalar*>(grad_out_ptr);
            auto* RESTRICT grad_in_data = reinterpret_cast<Scalar*>(grad_in_ptr);

            auto idx_dim = *reinterpret_cast<i64*>(idx_dim_ptr);
            auto idx_last_dim = *reinterpret_cast<i64*>(idx_last_dim_ptr);
          ...
          for (i64 elem = 0; elem < nelems; ++elem) {
            ...
            i64 left_fold_idx = (idx_dim > size) ? (idx_dim - size) / step : 0;
...
pub fn unfold_backward_cpu_kernel(
    grad_out: &mut Tensor,
    grad_in:  &Tensor,
    dim:      i64,
    size:     i64,
    ...
    todo!();
      ...
      AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
        ...
        iter.dtype(),
        "unfold_backward_cpu", [&] {
          _unfold_backward_internal_kernel<Scalar>(
            iter,
            size,
            step,
            grad_in_dim_stride,
