...
pub mod autocast {
    ...
    /* Logic to extract the promote type from any Tensor or &[Tensor] args. */
    ...
    /**
      | Overload to catch &[Tensor] args (for
      | e.g. cat, stack).
      |
    ...
    */
    ...
    lazy_static!{
        ...
        TORCH_LIBRARY_IMPL(aten, Autocast, m) {
              ...
              KERNEL(ADD_NS(chain_matmul), "chain_matmul", Tensor (&[Tensor]), lower_precision_fp)
              KERNEL(ADD_NS(linalg_multi_dot), "linalg_multi_dot", Tensor (&[Tensor]), lower_precision_fp)
              ...
              m.impl("lstm_cell",
                     TORCH_FN((&WrapFunction<CastPolicy::lower_precision_fp, DeviceType::Cuda,
                                             tuple<Tensor,Tensor> (const Tensor &, &[Tensor], const Tensor &, const Tensor &, const optional<Tensor>&, const optional<Tensor>&),
                                             tuple<Tensor,Tensor> (const Tensor &, &[Tensor], const Tensor &, const Tensor &, const optional<Tensor>&, const optional<Tensor>&),
              ...
              KERNEL(ADD_NS(cat), "cat", Tensor (&[Tensor], i64), promote)
              KERNEL(ADD_NS(cat), "cat.names", Tensor (&[Tensor], Dimname), promote)
              KERNEL(ADD_NS(_cat), "_cat", Tensor (&[Tensor], i64), promote)
              ...
              KERNEL(ADD_NS(stack), "stack", Tensor (&[Tensor], i64), promote)
    ...
    lazy_static!{
        /*
            TORCH_LIBRARY_IMPL(aten, AutocastCPU, m) {
              ...
              // promote
              KERNEL_CPU(ADD_NS(cat), "cat", Tensor (&[Tensor], i64), promote)
              KERNEL_CPU(ADD_NS(stack), "stack", Tensor (&[Tensor], i64), promote)
