...
#[inline] pub fn has_output(blob: &String, op: &OperatorDef) -> bool {
...
#[inline] pub fn rewrite_subnets_for_if_op(
    from: &String,
    to:   &String,
    op:   *mut OperatorDef)  {
...
#[inline] pub fn rename_inputs(
    from:     &String,
    to:       &String,
    def:      *mut OperatorDef,
    op_idx:   i32,
    children: &mut HashMap<String,HashSet<i32>>)  {
...
#[inline] pub fn rename_outputs(
    from:    &String,
    to:      &String,
    def:     *mut OperatorDef,
    op_idx:  i32,
    parents: &mut HashMap<String,HashSet<i32>>)  {
...
#[inline] pub fn rename_inputs_in_children(
    from:     &String,
    to:       &String,
    net:      *mut NetDef,
    children: &mut HashMap<String,HashSet<i32>>)  {
...
#[inline] pub fn rename_output_in_parents(
    from:    &String,
    to:      &String,
    net:     *mut NetDef,
    parents: &mut HashMap<String,HashSet<i32>>)  {
...
#[inline] pub fn found_op_candidate(
    op:       *const OperatorDef,
    op_idx:   i32,
    op_type:  &String,
    inputs:   &HashSet<String>,
    outputs:  &HashSet<String>,
    parents:  &HashMap<String,HashSet<i32>>,
    children: &HashMap<String,HashSet<i32>>) -> bool {
...
/**
  | Conceptually it's a pretty easy process
  | and consists of 3 steps:
  | 
  | -1) SSA rewrite;
  | 
  | -2) propagate inputs forwards;
  | 
  | -3) propagate outputs backwards and
  | then forwards again.
  | 
  | However, because of model outputs which
  | can't be overwritten during the SSA
  | process, and the fact that inputs could
  | be overwritten by ops and also appear
  | in outputs, it adds a lot of extra complexity
  | to handle these special cases.
  | 
  | A lot of this extra complexity is handled
  | in
  | 
  | FoundOpCandidate.
  |
  */
#[inline] pub fn remove_ops_by_type(graph: &mut InferenceGraph, op_type: &String)  {
