...
impl HeatmapMaxKeypointOp<f32, CPUContext> {
    
    /**
      | Mask R-CNN uses bicubic upscaling before
      | taking the maximum of the heat map for
      | keypoints.
      | 
      | We would like to avoid bicubic upscaling,
      | because it is computationally expensive.
      | 
      | This approach uses the Taylor expansion
      | up to the quadratic terms on approximation
...
pub type HeatmapMaxKeypointOpFloatCPU = HeatmapMaxKeypointOp<f32, CPUContext>;
