/*!
  ...
  |
  | A *batching rule* implements the logic of how
  | to call an operator on inputs that have zero or
  | more additional batch dimensions. When one does
  | a vmap, the dimension(s) being vmap'ed over get
  | recorded as batch dimensions.
  |
  | For example, vmap(torch.add)(x, y)
  |
  | 1. wraps `x` into batched_x = BatchedTensor(x,
  | bdims=[(lvl=1, dim=0)];
  |
  | 2. wraps `y` into batched_y = BatchedTensor(y,
  | bdims=[(lvl=1, dim=0)];
  |
  | 3. and then runs `torch.add(batched_x,
  |    batched_y)`.
  | NOTE: [When should I add a batching rule?]
  |
  | When you are adding a new operator, you'll need
  | to add a batching rule so that vmap can work
  | efficiently with said operator. If you do not,
  | we'll attempt to generate a slow fallback for
  | the batching rule.
  | NOTE: [How to write batching rules?]
  |
  | The signature of a batching rule should look
  | like exactly like the C++ signature of its
  | operator.
  |
  | First, see NOTE: [Logical vs physical args] in
  | VmapTransforms.h for terminology.
  |
  | At a high level, what a batching rule does is
  | the following:
  |
  | 1. Converts (logical) BatchedTensors to views
  | on physical tensors.
  |
  | 2. Converts logical arguments (e.g. dimension
  |    indexes, shapes) to physical arguments that
  |    correspond to the physical tensors.
  |
  | 3. Calls at:: operations on the physical
  |    tensors and arguments to produce some
  |    physical results.
  |
  | 4. Converts physical results back to
  | BatchedTensors.
  |
  | Steps 1, 2, and 4 differ for operators with
  | different batching behaviors. When writing
  | a new batching rule, please select
  | a VmapTransform that matches the batching
  | behavior of your operation. The VmapTransform
  | provides helper functions to do steps (1), (2),
  | and (4).
  |
  | (see NOTE: [What is an VmapTransform?] in
  | VmapTransforms.h)
  | Note: [Future plans]
  |
  | The API for writing a batching rule isn't
  | stable. In the future, we'd like to think about
  | the problem of translating these batching rules
  | to TorchScript.
  |
  | Ideally batching rules in eager mode vs
  | TorchScript would look pretty similar, if not
  | use the same mechanism. In order to accomplish
  | that we might have to do some refactoring.
  */

crate::ix!();

//-------------------------------------------[.cpp/pytorch/aten/src/ATen/BatchingRegistrations.cpp]

/**
  | PyTorch allows operations to specify
  | dim 0 and dim -1 on a scalar tensor.
  |
  */
pub fn is_allowed_dim_on_scalar_tensor(dim: i64) -> bool {
...
pub fn is_physical_scalar_tensor(logical_tensor: &Tensor) -> bool {
...
pub fn binary_pointwise_batching_rule(
        self_: &Tensor,
        other: &Tensor,
        args:  ExtraArgs) -> Tensor {
...
pub fn expand_batching_rule(
        self_:    &Tensor,
        size:     &[i32],
        implicit: bool) -> Tensor {
...
pub fn chunk_batching_rule(
    ...
    todo!();
      ...
      auto result = chunk(self_physical.tensor(), chunks, dim_physical);
      self_physical.getPhysicalToLogicalMap().applyInplace(result);
      return result;
        */
}

pub fn clamp_batching_rule(
        self_: &Tensor,
        min:   &Option<Scalar>,
    ...
    todo!();
        /*
            auto self_physical = MultiBatchVmapTransform::logicalToPhysical(self);
      auto result = clamp(self_physical.tensor(), min, max);
      return self_physical.getPhysicalToLogicalMap().apply(result);
        */
}

pub fn clamp_min_batching_rule(
        self_: &Tensor,
        min:   &Scalar) -> Tensor {
    ...
    todo!();
        /*
            auto self_physical = MultiBatchVmapTransform::logicalToPhysical(self);
      auto result = clamp_min(self_physical.tensor(), min);
      return self_physical.getPhysicalToLogicalMap().apply(result);
        */
}

pub fn clamp_max_batching_rule(
        self_: &Tensor,
        max:   &Scalar) -> Tensor {
    ...
    todo!();
        /*
            auto self_physical = MultiBatchVmapTransform::logicalToPhysical(self);
      auto result = clamp_max(self_physical.tensor(), max);
      return self_physical.getPhysicalToLogicalMap().apply(result);
        */
}

pub fn tensor_split_sections_batching_rule(
        self_:    &Tensor,
        sections: i64,
        dim:      i64) -> Vec<Tensor> {
    ...
    todo!();
        /*
            auto self_physical = MultiBatchVmapTransform::logicalToPhysical(self);
      auto dim_physical = self_physical.getPhysicalDim(dim);
      auto result = tensor_split(self_physical.tensor(), sections, dim_physical);
      self_physical.getPhysicalToLogicalMap().applyInplace(result);
      return result;
        */
}

pub fn tensor_split_indices_batching_rule(
        self_:   &Tensor,
        indices: &[i32],
        dim:     i64) -> Vec<Tensor> {
    ...
    todo!();
        /*
            auto self_physical = MultiBatchVmapTransform::logicalToPhysical(self);
      auto dim_physical = self_physical.getPhysicalDim(dim);
      auto result = tensor_split(self_physical.tensor(), indices, dim_physical);
      self_physical.getPhysicalToLogicalMap().applyInplace(result);
      return result;
        */
}

pub fn unsqueeze_batching_rule(
        self_: &Tensor,
        dim:   i64) -> Tensor {
...
pub fn fill_inplace_scalar_batching_rule<'a>(
        self_: &'a mut Tensor,
        value: &'a Scalar) -> &'a mut Tensor {
...
pub fn fill_inplace_tensor_batching_rule<'a>(
    self_: &'a mut Tensor,
    value: &'a Tensor) -> &'a mut Tensor {
...
pub fn zero_inplace_batching_rule<'a>(self_: &'a mut Tensor) -> &'a mut Tensor {
...
pub fn squeeze_batching_rule(self_: &Tensor) -> Tensor {
    ...
    todo!();
      ...
      i64 num_batch_dims = self_physical.numBatchDims();
...
pub fn squeeze_dim_batching_rule(
        self_: &Tensor,
        dim:   i64) -> Tensor {
...
pub fn trace_batching_rule(self_: &Tensor) -> Tensor {
    
    todo!();
        /*
            auto self_physical = MultiBatchVmapTransform::logicalToPhysical(self);
      // Batched Diagonal View
      auto self_diag = diagonal(self_physical.tensor(), /*offset*/0, /*dim1*/-2, /*dim2*/-1);
      auto result =  sum(self_diag, -1);
      return self_physical.getPhysicalToLogicalMap().apply(result);
        */
}

pub fn trace_backward_batching_rule(
        grad:        &Tensor,
        input_sizes: &[i32]) -> Tensor {
    
    todo!();
        /*
            auto grad_physical = MultiBatchVmapTransform::logicalToPhysical(grad);
      auto grad_input = zeros(grad_physical.getPhysicalShape(input_sizes), grad.options());
      // Batched Diagonal View
      auto grad_input_diag = diagonal(grad_input, /*offset*/0, /*dim1*/-2, /*dim2*/-1);
...
pub fn transpose_int_batching_rule(
        self_: &Tensor,
        dim0:  i64,
        dim1:  i64) -> Tensor {
...
pub fn permute_batching_rule(
    ...
    todo!();
      ...
      for (i64 bdim = 0; bdim < self_physical.numBatchDims(); bdim++) {
...
pub fn get_grad_input_physical_dim(
        dim:            i64,
        input_sizes:    &[i32],
        num_batch_dims: i64) -> i64 {
...
pub fn select_backward_batching_rule(
        grad:        &Tensor,
        input_sizes: &[i32],
        dim:         i64,
        index:       i64) -> Tensor {
...
pub fn slice_backward_batching_rule(
        grad:        &Tensor,
        input_sizes: &[i32],
        dim:         i64,
        start:       i64,
...
pub fn diagonal_batching_rule(
    ...
    todo!();
      ...
      auto result = diagonal(self_physical.tensor(), offset, dim1_physical, dim2_physical);
      return self_physical.getPhysicalToLogicalMap().apply(result);
        */
}

pub fn diagonal_backward_batching_rule(
        grad:        &Tensor,
        input_sizes: &[i32],
        offset:      i64,
        dim1:        i64,
...
pub fn movedim_batching_rule(
        self_:       &Tensor,
        source:      &[i32],
        destination: &[i32]) -> Tensor {
    ...
    todo!();
      ...
      auto result = movedim(self_physical.tensor(), source_physical, destination_physical);
      return self_physical.getPhysicalToLogicalMap().apply(result);
        */
}

pub fn reshape_batching_rule(
        self_: &Tensor,
        shape: &[i32]) -> Tensor {
...
pub fn split_batching_rule(
    ...
    todo!();
        /*
            auto self_physical = MultiBatchVmapTransform::logicalToPhysical(self);
      auto dim_physical = self_physical.getPhysicalDim(dim);
      auto result = split(self_physical.tensor(), split_size, dim_physical);
      self_physical.getPhysicalToLogicalMap().applyInplace(result);
      return result;
        */
}

pub fn split_with_sizes_batching_rule(
        self_:       &Tensor,
        split_sizes: &[i32],
        dim:         i64) -> Vec<Tensor> {
...
pub fn unbind_batching_rule(
    ...
    todo!();
        /*
            auto self_physical = MultiBatchVmapTransform::logicalToPhysical(self);
      auto dim_physical = self_physical.getPhysicalDim(dim);
      auto result = unbind(self_physical.tensor(), dim_physical);
      self_physical.getPhysicalToLogicalMap().applyInplace(result);
      return result;
        */
}

pub fn unfold_batching_rule(
        self_: &Tensor,
        dim:   i64,
        size:  i64,
        step:  i64) -> Tensor {
...
pub fn contiguous_batching_rule(
        self_:         &Tensor,
...
pub fn view_as_complex_batching_rule(self_: &Tensor) -> Tensor {
    ...
    todo!();
      ...
      auto self_physical = MultiBatchVmapTransform::logicalToPhysical(self);
      auto result = view_as_complex(self_physical.tensor());
      return self_physical.getPhysicalToLogicalMap().apply(result);
        */
}

/**
  | Checks that the smallest batch stride is
  | greater than the largest example stride. This
  | is something we can support but we choose not
  | to because it's potentially error prone.
  |
  */
pub fn check_batch_dims_at_front_in_layout(
        physical_strides: &[i32],
        num_batch_dims:   i64)  {
    
    todo!();
        /*
            auto smallest_batch_stride = min_element(
          physical_strides.begin(), physical_strides.begin() + num_batch_dims);
      auto largest_example_stride = max_element(
...
/**
  | given (sizes, strides, storage_offset) returns
  | the maximum location that can be indexed (or
  | nullopt if such a location doesn't exist, e.g.,
  | tensors with zero-size dims).
  |
  */
pub fn maximum_indexable_location(
        sizes:          &[i32],
        strides:        &[i32],
        storage_offset: i64) -> Option<i64> {
...
/**
  ...
  |
  | This checks that the range of possible memory
  | locations accessible by x.as_strided(sizes,
  | strides, maybe_storage_offset) are within the
  | bounds of possible memory locations accessible
  | by x.
  |
  */
pub fn check_basic_as_strided_valid_for_slice(
        physical_tensor:      &Tensor,
        num_batch_dims:       i64,
        sizes:                &[i32],
        strides:              &[i32],
        maybe_storage_offset: Option<i64>)  {
...
/**
  ...
  |
  */
pub fn as_strided_batching_rule(
        tensor:         &Tensor,
        sizes:          &[i32],
        strides:        &[i32],
...
/**
  | NOTE: [When will the as_strided batching rule
  | fail?]
  |
  | If zi = xs[i].as_strided(sizes, strides, offset
  | + xs[i].offset() - xs.offset()) is valid for
  | all i, then it turns out that
  | xs.as_strided(physical_sizes, physical_strides,
  | offset) always succeeds and creates a tensor
  | y such that each y[i] refers to the same memory
  | as zi.
  |
  | Let's say we have xs[i].as_strided(sizes,
  | strides, offset + xs[i].offset()
  | - xs.offset()).
  |
  | Furthermore, let's say that as a part of being
  | "valid" this as_strided call does not return
  | a result that can index memory not indexable by
  | xs[i].
  |
  | WLOG, assume that there's only one batch dim
  | and it is at the front of the `xs` tensor. Let
  | B be the batch size and S be the stride of the
  | batch dim.
  |
  | - If the batch dim isn't at the front of the
  | tensor, then we can just move it to the front
  | with movedim/permute. This is always valid
  | because it just swaps some strides around.
  |
  | - This proof also works for tensors with
  | multiple batch dims. We just have to
  |
  ...
  | [Equation 1]
  |
  | xs[i].as_strided(sizes, strides, offset
  | + xs[i].offset() - xs.offset()) has:
  |
  | - sizes: sizes
  | - strides: strides
  | - offset: offset + S * i
  ...
  | - offset is positive
  |
  | Claim 1: if xs[i].as_strided(sizes, strides,
  | offset + xs[i].offset() - xs.offset()) is
  | valid, then
  |
  | ([B] + sizes, [S] + strides, offset
  | + xs.offset()) are in bounds for `xs`'s
  | storage.
  |
  | If we have the claim, then xs.as_strided([B]
  | + sizes, [S] + strides, offset) won't error
  | out. So all we need to check is that the memory
  | locations are what we expected. See [Hand-wavy
  | proof of Claim 1] for proof (it's not very
  | important)
  |
  ...
  | - offset: offset
  |
  | xs.as_strided([B] + sizes, [S] + strides,
  | offset)[i] has:
  |
  | - sizes: sizes
  | - strides: strides
  | - offset: offset + S * i
  |
  | These memory locations are exactly the same as
  | what we got for [Equation 1], so the
  | xs.as_strided([B] + sizes, [S] + strides,
  | offset) is valid.
  |
  | [Hand-wavy proof of Claim 1]
  |
  | Part of our definition of being valid is that
  | xs[i].as_strided(...) must return a tensor that
  | only uses memory indexable by xs[i].
  |
  | This means that (sizes, strides, offset
  | + xs[i].offset() - xs.offset()) satisfies:
  |
  |    offset + xs[i].offset() - xs.offset() + 1 + \sum_j (sizes[j] - 1) * strides[j]
  |    <= xs[i].offset() + 1 + \sum_j (xs[i].size(j) - 1) * xs[i].stride(j)
  |
  | (the largest-index memory location of
  | xs[i].as_strided(...) must be \leq the
  | largest-index memory location of xs[i])
  |
  ...
  |
  | (the largest-index memory location of
  | xs.as_strided(size, stride, offset) is \leq
  | than the largest-index memory location of xs)
  |
  | Under the assumptions we've made, the lower
  | bound (lowest indexed memory) is trivially
  | within the storage.
  |
  | Therefore ([B] + sizes, [S] + strides, offset)
  | are in bounds for `xs`'s storage.
  */
pub fn unwrap_and_call(
        input: &Tensor,
        args:  ExtraArgs) -> Tensor {
...
pub fn unwrap_and_call_method(
        input:      &Tensor,
        extra_args: ExtraArgs) -> Tensor {
...
pub fn pow_scalar_tensor_batching_rule(
        other: &Scalar,
        self_: &Tensor) -> Tensor {
    ...
    todo!();
      ...
      auto output_physical = pow(other, self_batched->value());
      auto old_bdims = self_batched->bdims();
      return makeBatched(output_physical, BatchDims(old_bdims.begin(), old_bdims.end()));
        */
}

pub fn clone_batching_rule(
        self_:         &Tensor,
    ...
    todo!();
      ...
      if (memory_format == MemoryFormat::Contiguous) {
        ...
        auto physical_view = MultiBatchVmapTransform::logicalToPhysical(self);
        auto output_physical = clone(physical_view.tensor(), memory_format);
      ...
      auto* self_batched = unsafeGetBatchedImpl(self);
      auto output_physical = clone(self_batched->value(), memory_format);
      auto old_bdims = self_batched->bdims();
      return makeBatched(output_physical, BatchDims(old_bdims.begin(), old_bdims.end()));
        */
}
...
/**
  ...
  | matmul doesn't "de-expand" arguments to get
  | better performance (maybe it should). In the
  | batching rules for matmul-like operators (dot,
  | mv, mm), we should be careful not to expand any
  | unnecessary dimensions. e.g., if only one of
  | the two arguments is a BatchedTensor, then we
  | should try not to expand batch dimensions onto
  | the other arg.
  |
  */
pub fn mv_batching_rule(
        self_: &Tensor,
        other: &Tensor) -> Tensor {
    ...
    todo!();
      ...
      if (self_batched && !other_batched) {
        auto self_physical = MultiBatchVmapTransform::logicalToPhysical(self);
        auto result = matmul(self_physical.tensor(), other);
        return self_physical.getPhysicalToLogicalMap().apply(result);
      }
      if (!self_batched && other_batched) {
        ...
        auto other_physical = MultiBatchVmapTransform::logicalToPhysical(other);
        auto result = matmul(self, other_physical.tensor().unsqueeze(-1));
...
pub fn dot_batching_rule(
    ...
    todo!();
      ...
      if (self_batched && !other_batched) {
        ...
        auto self_physical = MultiBatchVmapTransform::logicalToPhysical(self);
        auto result = matmul(self_physical.tensor().unsqueeze(-2), other);
...
pub fn bmm_batching_rule(
    ...
    todo!();
      ...
      auto physical_args = BroadcastingVmapTransform::logicalToPhysical({self, other});
      auto result = matmul(physical_args[0].tensor(), physical_args[1].tensor());
      return physical_args[0].getPhysicalToLogicalMap().apply(result);
        */
}

pub fn mm_batching_rule(
        self_: &Tensor,
        other: &Tensor) -> Tensor {
    ...
    todo!();
      ...
      if (!self_batched && other_batched) {
        auto other_physical = MultiBatchVmapTransform::logicalToPhysical(other);
        auto result = matmul(self, other_physical.tensor());
        return other_physical.getPhysicalToLogicalMap().apply(result);
      }
      if (self_batched && other_batched) {
        auto physical_args = MultiBatchVmapTransform::logicalToPhysical({self, other});
...
pub fn cat_batching_rule(
    ...
    todo!();
      ...
      auto result = cat(physical_tensors, physical_views[0].getPhysicalDim(dim));
      return physical_views[0].getPhysicalToLogicalMap().apply(result);
        */
}

pub fn stack_batching_rule(
        tensors: &[Tensor],
        dim:     i64) -> Tensor {
    ...
    todo!();
      ...
      auto result = stack(physical_tensors, dim_physical);
      return physical_views[0].getPhysicalToLogicalMap().apply(result);
        */
}

/**
  | I am quite sad that we need to register
  | operators with exploded TensorOptions, even
  | though the native:: implementations can use
  | TensorOptions&.
  |
  | This also makes it hard to metaprogram: i.e.,
  | we can't use unwrap_and_call<..., to> because
  | to takes TensorOptions& (!!)
  |
  */
pub fn to_dtype_layout_batching_rule(
        self_:         &Tensor,
...
pub fn new_zeros_batching_rule(
        self_:      &Tensor,
        size:       &[i32],
        dtype:      Option<ScalarType>,
        layout:     Option<Layout>,
...
pub fn new_empty_batching_rule(
        self_:      &Tensor,
        size:       &[i32],
        dtype:      Option<ScalarType>,
        layout:     Option<Layout>,
...
pub fn new_empty_strided_batching_rule(
        self_:      &Tensor,
        size:       &[i32],
        stride:     &[i32],
        dtype:      Option<ScalarType>,
    ...
    todo!();
      ...
      auto physical_strides = detail::defaultStrides(batch_shape);
...
pub fn comparison_pointwise_batching_rule(
        self_: &Tensor,
        other: &Tensor) -> Tensor {
...
lazy_static!{
    ...
    TORCH_LIBRARY_IMPL(aten, Batched, m) {
      ...
      m.impl("size.int", static_cast<i64 (*)(const Tensor&, i64)>(native::size));
      ...
      m.impl("movedim.int", static_cast<Tensor(*)(const Tensor&,i64,i64)>(native::movedim)); // composite wrt autograd
      ...
      m.impl("narrow", static_cast<Tensor(*)(const Tensor&,i64,i64,i64)>(native::narrow)); // composite wrt autograd
...
/**
  | unary pointwise, out-of-place, no
...
lazy_static!{
    ...
    #define UNARY_POINTWISE(op) m.impl(#op, \
        unwrap_and_call<Tensor (*)(const Tensor&), op>);
      UNARY_POINTWISE(abs);
    ...
    #define BINARY_POINTWISE(op) \
      m.impl(#op".Tensor", binary_pointwise_batching_rule<TensorTensorType, op>); \
      m.impl(#op".Scalar", unwrap_and_call<TensorScalarType, op, const Scalar&>);
    #define BINARY_POINTWISE_VA(op, ...) \
      { \
        ...
        m.impl(#op".Tensor", binary_pointwise_batching_rule<Binop, op, __VA_ARGS__>); \
        m.impl(#op".Scalar", unwrap_and_call<Unop, op, const Scalar&, __VA_ARGS__>); \
      }
      ...
      {
        using Binop = Tensor (*)(const Tensor&, const Tensor&, optional<string_view>);
        using Unop = Tensor (*)(const Tensor&, const Scalar&, optional<string_view>);
        m.impl("div.Tensor_mode", binary_pointwise_batching_rule<Binop, div, optional<string_view>>);
        m.impl("div.Scalar_mode", unwrap_and_call<Unop, div, const Scalar&, optional<string_view>>);
      }

      // pow has three out-of-place overloads
      m.impl("pow.Tensor_Tensor", binary_pointwise_batching_rule<TensorTensorType, pow>);
      m.impl("pow.Tensor_Scalar", unwrap_and_call<TensorScalarType, pow, const Scalar&>);
      m.impl("pow.Scalar", pow_scalar_Tensor_batching_rule);

      m.impl("sigmoid_backward", binary_pointwise_batching_rule<TensorTensorType, sigmoid_backward>);
      m.impl(
          "threshold_backward",
      ...
      // for result_type, call the native::result_type implementation.
    ...
    #define COMPARISON_POINTWISE(op) \
      m.impl(#op".Tensor", comparison_pointwise_batching_rule<TensorTensorType, op>); \
      m.impl(#op".Scalar", unwrap_and_call<TensorScalarType, op, const Scalar&>);
