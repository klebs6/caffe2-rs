...
pub struct TvmTransformOptions {
    ...
    profiling_based_jit:  bool, // default = false
}

impl TvmTransformOptions {
...
///---------------------------
pub struct TvmTransformer {
    base: BackendTransformerBase,

    /// Options
    opts:  TvmTransformOptions,

    /// Track number of TVMJitOp we created
    tvm_op_id:  i32, // default = 0
...
impl TvmTransformer {
    ...
    /**
      | Given TVM runnable subnets, contract
      | them into one TVMJitOp
      |
      */
    #[inline] pub fn build_tvm_op(&mut self, 
        net:         &NetDef,
        weights:     &HashSet<String>,
        shape_hints: &ShapeInfoMap) -> NetDef {
    ...
    /**
      | Given workspace and predict net, cluster
      | continuous parts that can be run by TVM
      | and create one TVMJit op for each clustered
      | subgraph.
      | 
      | -----------
      | @param ws
      ...
      | @param weight_names
      | 
      | list of the names of the constant weights
      | ----------
      | @param shape_hints
      | 
      | User provided shape info, usually for
      | primary inputs so that bound shape inference
      | can have something to start
      | ----------
      | @param blocklisted_ops
      | 
      | a set of ops that we don't want to lower
      | to TVM in terms of their net positions.
      | This is very useful for debugging but
      | for normal runs it should be empty
      | 
      | Cutting off the runnable part and replace
      | with TVMJitOPs. Asssume the nets were
    ...
    #[inline] pub fn get_supported_ops(&mut self) -> &HashSet<String> {
    ...
    #[inline] pub fn can_convert_full_graph(&mut self, net: &NetDef, blocklisted_ops: &HashSet<i32>) -> bool {
    ...
    #[inline] pub fn apply_tvm_transform(&mut self, 
        pred_net:        *mut NetDef,
        weights:         &HashSet<String>,
        blocklisted_ops: &HashSet<i32>,
        shape_hints:     &ShapeInfoMap) -> NetDef {
...
#[inline] pub fn clean_up_predict_net(
    net:          *mut NetDef,
    input_names:  &Vec<String>,
    output_names: &Vec<String>,
    weight_names: &Vec<String>)  {
