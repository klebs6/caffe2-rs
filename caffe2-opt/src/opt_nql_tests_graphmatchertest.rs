...
/**
  ...
  |
  */
#[inline] pub fn gen_tensors(graph: &mut NNGraph, names: Vec<String>) -> HashMap<String,NNGraph_NodeRef> {
    
    todo!();
    /*
        std::unordered_map<std::string, NNGraph_NodeRef> result;
      for (auto& name : names) {
       ...
       The graph we're building looks like this:
        ...
        Concat
         ...
         Relu
           ...
           c
       */
      std::unordered_map<std::string, NNGraph_NodeRef> tensors;
