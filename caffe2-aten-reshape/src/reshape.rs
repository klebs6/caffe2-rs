...
/**
  ...
  |
  */
pub fn set_storage_cpu<'a>(
        result:         &mut Tensor,
        storage:        Storage,
        storage_offset: i64,
        size:           &[i32],
        stride:         &[i32]) -> &'a mut Tensor {
...
/**
  ...
  |
  */
pub fn set_cpu<'a>(result: &mut Tensor) -> &'a mut Tensor {
...
pub fn broadcast_tensors(tensors: &[Tensor]) -> Vec<Tensor> {
...
pub fn cat_out_cpu<'a>(
    ...
    todo!();
      ...
      const Tensor* pnotSkippedTensor = [](&[Tensor] tensors) -> const Tensor* {
...
pub fn check_cat_no_zero_dim(tensors: &[Tensor])  {
...
pub fn block_diag(tensors: &[Tensor]) -> Tensor {
...
pub fn as_strided<'a>(
        ...
        size:           &[i32],
        stride:         &[i32],
        storage_offset: Option<i64>) -> &'a Tensor {
...
pub fn narrow_copy_dense_cpu_out<'a>(
        self_:  &Tensor,
        dim:    i64,
        start:  i64,
        length: i64,
    ...
    todo!();
        /*
            TORCH_CHECK(tensors.size() > 0,
               "stack expects a non-empty &[Tensor]");
      dim = maybe_wrap_dim(dim, tensors[0].dim() + 1);
...
pub fn hstack(tensors: &[Tensor]) -> Tensor {
    ...
    todo!();
        /*
            TORCH_CHECK(tensors.size() > 0,
               "hstack expects a non-empty &[Tensor]");
...
pub fn vstack(tensors: &[Tensor]) -> Tensor {
    ...
    todo!();
        /*
            TORCH_CHECK(tensors.size() > 0,
               "vstack expects a non-empty &[Tensor]");
...
pub fn dstack(tensors: &[Tensor]) -> Tensor {
    
    todo!();
        /*
            TORCH_CHECK(tensors.size() > 0,
               "dstack expects a non-empty &[Tensor]");
...
#[inline] pub fn sparse_transpose<'a>(
        self_: &mut Tensor,
        dim0:  i64,
...
pub fn row_stack(tensors: &[Tensor]) -> Tensor {
...
pub fn reshape_input_for_column_stack(tensors: &[Tensor]) -> Vec<Tensor> {
...
pub fn column_stack_out<'a>(
    ...
    todo!();
        /*
            TORCH_CHECK(tensors.size() > 0,
                  "column_stack expects a non-empty &[Tensor]");
...
pub fn column_stack(tensors: &[Tensor]) -> Tensor {
...
pub fn propagate_transposed_names<'a>(
        result: &mut Tensor,
        other:  &Tensor,
        dim0:   i64,
        dim1:   i64) -> &'a mut Tensor {
...
pub fn t_a<'a>(self_: &Tensor) -> Tensor {
...
pub fn t_b<'a>(self_: &mut Tensor) -> &'a mut Tensor {
...
pub fn squeeze_c<'a>(self_: &mut Tensor) -> &'a mut Tensor {
...
pub fn meshgrid(tensors: &[Tensor]) -> Vec<Tensor> {
    
    todo!();
        /*
            i64 size = tensors.size();
      TORCH_CHECK(size > 0, "meshgrid expects a non-empty &[Tensor]");
