...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/core/boxing/impl/WrapFunctionIntoRuntimeFunctor.h]
...
/**
  | WrapFunctionIntoRuntimeFunctor: Wraps any
  | runtime functor into a functor that inherits
  | from c10::OperatorKernel, so it can be used
  | as a c10 kernel.
  |
  | This can, for example, be used for lambdas,
  | functors or even function pointers.
  |
  | In the case of function pointers, since it is
  | a runtime function pointer, there is an
  | overhead for calling it whenever the kernel
  | is invoked.
  */
lazy_static!{
    /*
