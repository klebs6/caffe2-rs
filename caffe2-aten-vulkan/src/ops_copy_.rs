...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/vulkan/ops/Copy.h]
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/vulkan/ops/Copy.cpp]
...
pub fn copy_<'a>(
    ...
    todo!();
      ...
      {
        // X -> Vulkan
        if (kVulkan == self.device().type()) {
          vTensor& v_self = convert(self);

          // Vulkan -> Vulkan
          if (kVulkan == src.device().type()) {
            command_buffer.copy(
          ...
          // CPU -> Vulkan
            ...
            memcpy(
                ...
                min(src.nbytes(), self.nbytes()));
          }
        }
        // Vulkan -> X
        else if (kVulkan == src.device().type()) {
