...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/quantized/cpu/qnnpack/src/tanh.c]
...
pub fn pytorch_qnnp_create_tanh_nc_q8(
    ...
    output_max:        u8,
    flags:             u32,
    tanh_out:          *mut PyTorchQnnpOperator) -> PyTorchQnnpStatus {
    ...
    /*
      ...
      tanh_op->lookup_table = malloc(256 * sizeof(u8));
      ...
      u8* lookup_table = tanh_op->lookup_table;
      ...
      for (i32 i = 0; i < 256; i++) {
        ...
        */
        ...
        lookup_table[(u32)i] = (u8)lrintf(scaled_tanh_x);
      }
