...
#[test] fn batch_bucketize_op_example() {
...
/**
  ...
  | of elements in `indices` should be less
  | than or equal to `D`.
  | 
...
num_inputs!{BatchBucketize, 4}
num_outputs!{BatchBucketize, 1}

inputs!{BatchBucketize, 
    0 => ("float_features", "2-D dense tensor, the second dimension must be greater or equal to the indices dimension"),
    1 => ("indices", "Flatten tensor, containing the indices of `float_features` to be bucketized. The datatype must be int32."),
    2 => ("lengths", "Flatten tensor, the size must be equal to that of `indices`. The datatype must be int32."),
    3 => ("boundaries", "Flatten tensor, dimension has to match the sum of lengths")
}

outputs!{BatchBucketize, 
    0 => ("bucktized_feat", 
...
impl<Context> BatchBucketizeOp<Context> {
...
impl BatchBucketizeOp<CPUContext> {
