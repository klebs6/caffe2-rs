...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/FunctionOfAMatrixUtils.h]
...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/FunctionOfAMatrixUtils.cpp]
...
/**
  ...
  |
  | Note: if input.dtype == Scalar<T>, then
  | coefficients.dtype == T.
  |
  | This is relevant when Scalar<T> ==
  | complex<T>.
  |
  */
pub fn compute_linear_combination(
    ...
    todo!();
      ...
      auto output = zeros(
        output_sizes,
        input.options().memory_format(MemoryFormat::Contiguous)
      );
...
/**
  | Note: the function is implemented using the
  | __restrict__ memory modifier, which means that
  | if `output` actually is aliased by `input`, the
  | result produced is undefined.
  |
  */
pub fn compute_linear_combination_out<'a>(
        input:        &Tensor,
        coefficients: &Tensor,
    ...
    todo!();
      ...
      auto coefficients_restrided_sizes = vector<i64>(input.dim() + 1, 1);
      coefficients_restrided_sizes[0] = output_first_dim_size;
      coefficients_restrided_sizes[1] = 1;
      auto coefficients_restrided_strides = vector<i64>(input.dim() + 1, 0);
