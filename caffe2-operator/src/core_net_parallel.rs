...
declare_string!{caffe2_task_graph_engine}
...
pub struct ParallelNet {
    ...
    num_workers:     i32,
    helper:          Box<ParallelNetExecutorHelper>,
    task_graph:      Arc<dyn AsyncTaskGraphBase>,
    run_future:      *mut AsyncTaskFuture,
...
impl ParallelNet {
    ...
    pub fn new(net_def: &Arc<NetDef>, ws: *mut Workspace) -> Self {
      ...
      for (auto chain_id = 0U; chain_id < chains.size(); ++chain_id) {
        std::vector<OperatorStorage*> ops;
    ...
    #[inline] pub fn pool_getter(&mut self, 
        ...
        device_type: i32,
        device_id:   i32,
        pool_size:   i32) -> *mut dyn TaskThreadPoolBaseInterface {
    ...
    #[inline] pub fn pool(&mut self, device_option: &DeviceOption) -> Arc<dyn TaskThreadPoolBaseInterface> {
...
#[inline] pub fn get_async_task_graph(
    helper:  *mut ExecutorHelper, 
    options: &ExecutionOptions) -> Arc<dyn AsyncTaskGraphBase> {
