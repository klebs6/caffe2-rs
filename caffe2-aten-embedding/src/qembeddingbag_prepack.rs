...
//-------------------------------------------[.cpp/pytorch/aten/src/ATen/native/quantized/cpu/qembeddingbag_prepack.cpp]
...
/**
  | Prepack function for embedding_bag
  | weights.
  | 
  | This function expects a per-row quantized
  | weight tensor with a floating point
  | scale and zero_point value. zero point
  | is set to be (-Xmin/scale)
  | 
  | To prepack the weights we store the scale
  | and bias (where bias is Xmin) for each
  | row along with the quantized weights.
  |
  */
impl PackedEmbeddingBagWeight {
    
    pub fn prepack(&mut self, qweight: Tensor) -> IntrusivePtr<EmbeddingPackedParamsBase> {
        
        todo!();
        /*
            static constexpr i64 version = 1;
      TORCH_CHECK(
          qweight.dim() == 2,
          "quantized::embedding_bag_prepack weight tensor rank should be 2");
      TORCH_CHECK(
          qweight.scalar_type() == kQUInt8 ||
              qweight.scalar_type() == kQUInt4x2,
          "qembedding_bag_prepack currently only supports quint8 and quint4x2 weights");

      Tensor weight_contig =
          qweight.contiguous(qweight.suggest_memory_format());

      int bit_width, scale_bias_bytes;
      u8* weight_data = static_cast<u8*>(weight_contig.data_ptr());
      if (qweight.scalar_type() == kQUInt8) {
        bit_width = 8;
        scale_bias_bytes = 8; // extra 8 bytes to store FP scale and bias per row.
      } else {
        bit_width = 4;
        scale_bias_bytes =
            4; // extra 4 bytes to store Half scale and bias per row.
      }
      const auto num_elem_per_byte = 8 / bit_width;

      i64 embedding_rows = qweight.size(0);
      i64 embedding_cols = qweight.size(1);
      const auto qtype = qweight.qscheme();
      TORCH_CHECK(
          qtype == kPerChannelAffineFloatQParams,
          "Expect embedding_bag weights to be quantized using kPerChannelAffineFloatQParams");
      vector<float> weight_bias(embedding_rows, 0);
      vector<float> weight_scales(embedding_rows, 1.0);
      vector<float> weight_zero_points(embedding_rows, 0);

      for (i64 i = 0; i < embedding_rows; ++i) {
      ...
      usize output_columns = output_shape[1];

      // Allocate output packed weights.
      Tensor output = empty(
          output_shape,
          weight_contig.options().dtype(kByte),
          weight_contig.suggest_memory_format());
      auto* output_data = output.data_ptr<u8>();

      if (bit_width == 8) {
        parallel_for(
            0, embedding_rows, 1, [&](i32 start_idx, i32 end_idx) {
              for (i64 row = start_idx; row < end_idx; ++row) {
                const u8* input_row = weight_data + row * embedding_cols;
                u8* output_row = output_data + row * output_columns;
                ...
                for (i64 col = 0; col < embedding_cols; ++col) {
        ...
        parallel_for(
            0, embedding_rows, 1, [&](i32 start_idx, i32 end_idx) {
              for (i64 row = start_idx; row < end_idx; ++row) {
                const u8* input_row = weight_data + row * embedding_cols;
                u8* output_row = output_data + row * output_columns;
                Half* output_row_scale_bias =
                    reinterpret_cast<Half*>(output_row + embedding_cols);
                output_row_scale_bias[0] = weight_scales[row];
                output_row_scale_bias[1] = weight_bias[row];
                for (i64 col = 0; col < embedding_cols; ++col) {
      ...
      auto packed_ptr = make_intrusive<PackedEmbeddingBagWeight>(
...
/**
  | Note - This is a temporary pack function for
  | embedding bag which quantizes and packs the
  | float weight tensor. In the next step it will
  | be replaced by a quantize and pack function
  | once we support FP scale and FP zero_point
  |
  | Python example examining a packed 8bit
  | zero_point and scale:
  |
  ...
  |         [70.        , 80.00000035]]])
  */
pub fn qembeddingbag_byte_prepack(weight: &Tensor) -> Tensor {
    ...
    todo!();
      ...
      const i32 embedding_rows = Sizeo_dim_(cols_dim, weight_sizes);
      const i32 embedding_cols = weight_sizes[cols_dim];
      // Add 8 bytes per column to store FP32 scale and zero_point per row.
      const i32 output_columns = embedding_cols + 2 * sizeof(float);
      ...
      vector<i64> output_shape = weight_sizes.vec();
      ...
      for (usize row = 0; row < embedding_rows; ++row) {
        ...
        float minimum_element =
            *min_element(input_row, input_row + embedding_cols);
        float maximum_element =
            *max_element(input_row, input_row + embedding_cols);
        float range = maximum_element - minimum_element;

        output_row_scale_zp[0] = range / 255.0f;
        output_row_scale_zp[1] = minimum_element;
        const auto inverse_scale = 255.0f / (range + kEpsilon);
        for (usize col = 0; col < embedding_cols; ++col) {
          output_row[col] =
...
/**
  | TODO: Extend support to N-D batched
  | embeddings, similar to
  | qembeddingbag_byte_prepack
  |
  */
pub fn qembeddingbag_nbit_prepack_helper(
        weight:            &Tensor,
        bit_width:         i32,
        optimized_qparams: bool,
        nbins:             i64,
        ratio:             f64) -> Tensor {
    
    todo!();
        /*
            i64 embedding_rows = weight.size(0);
      i64 embedding_cols = weight.size(1);
      ...
      if (!optimized_qparams) {
        ...
        for (int row = 0; row < embedding_rows; ++row) {
          ...
          float Xmin, Xmax;
          if (optimized_qparams) {
            Tensor xmax_tensor, xmin_tensor;
            tie(xmax_tensor, xmin_tensor) = choose_qparams_optimized(
            ...
            Xmin = *min_element(input_row, input_row + embedding_cols);
            Xmax = *max_element(input_row, input_row + embedding_cols);
          }
          ...
          Half scale = range == 0 ? 1.0f : range / ((1 << bit_width) - 1);
          float inverse_scale = scale == 0 ? 1.0f : 1.0f / scale;
          if (scale == 0 || isinf(inverse_scale)) {
          ...
          Half* output_row_scale_zp = reinterpret_cast<Half*>(
              output_row +
              (embedding_cols + NUM_ELEM_PER_BYTE - 1) / NUM_ELEM_PER_BYTE);
          ...
          for (const auto col : irange(embedding_cols)) {
...
/**
  ...
  | minimum) and bias (minimum value) of
  | each row in the input matrix, and then
  ...
  |
  */
pub fn qembeddingbag_4bit_prepack(
        weight:            &Tensor,
        optimized_qparams: bool,
        nbins:             i64,
        ratio:             f64) -> Tensor {
...
/**
  ...
  |
  */
pub fn qembeddingbag_2bit_prepack(
        weight:            &Tensor,
        optimized_qparams: bool,
        nbins:             i64,
        ratio:             f64) -> Tensor {
...
pub struct QEmbeddingPackWeights {

}

impl QEmbeddingPackWeights {
    
    pub fn run(weight: Tensor) -> IntrusivePtr<EmbeddingPackedParamsBase> {
        
        todo!();
        /*
            return PackedEmbeddingBagWeight::prepack(move(weight));
        */
    }
}
