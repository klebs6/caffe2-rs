...
/**
  ...
  | accumulation.
  | 
  | We'll encounter saturation but this
  | will be faster in Intel CPUs
  | 
  | template <bool ReluFused = false>
  |
  */
pub struct ConvDNNLowPAcc16Op<const ReluFused: bool> {
    //USE_CONV_POOL_BASE_FUNCTIONS(CPUContext);
    base: ConvDNNLowPOp<u8, ReluFused>,

    wq_acc16_packed:                  Arc<PackBMatrix<i8,i16>>,

    /// Wq outlier in CSC format
    #[cfg(use_fbgemm)]
    wq_outlier:                       Arc<fbgemm::CompressedSparseColumn>,

    /**
      | Threshold to decide whether a weight is
      | outlier.
      |
      | For example, if nbits_in_non_outlier_ ==
      | 7, w is an outlier if w < -64 or w >= 64.
      |
      | nbits_in_non_outlier_ == 0 means
      | everything is outlier.
      |
      | nbits_in_non_outlier_ == 8 means nothing
      | is outlier.
      */
    nbits_in_non_outlier:             i32,

    copy_to_32bit_frequency:          i32,
    first_invocation:                 bool, // default = true

    /**
      | If outlier matrix is not sparse enough,
      | using 16-bit accumulation won't give
      | speedup due to too much overhead of sparse
      | matrix multiplication or sparse
      | convolution anyway, so fallback to 32-bit
      | accumulation
      */
    fallback_to_32_bit_accumulation:  bool, // default = false

    /*
      using BaseType = ConvDNNLowPOp<uint8_t, ReluFused>;
      using BaseType::BIAS;
...
declare_int32!{caffe2_dnnlowp_copy_to_32bit_frequency}
declare_bool!{caffe2_dnnlowp_shared_int32_buffer}

/**
  | Thresholds to fallback to 32-bit accumulation
  | when 16-bit accumulation doesn't provide
...
impl<const ReluFused: bool> ConvDNNLowPAcc16Op<ReluFused> {
    ...
    pub fn new(operator_def: &OperatorDef, ws: *mut Workspace) -> Self {
        ...
        /*
          ...
          nbits_in_non_outlier_(this->template GetSingleArgument<int>( "nbits_in_non_outlier", FLAGS_caffe2_dnnlowp_nbits_in_non_outlier)),
          copy_to_32bit_frequency_(this->template GetSingleArgument<int>( "copy_to_32bit_frequency", FLAGS_caffe2_dnnlowp_copy_to_32bit_frequency)) 
            ...
            for (int i = 0; i < M / group_; ++i) {
              for (int j = 0; j < output_image_size; ++j) {
                ...
                for (int k = 0; k < kernel_dim; ++k) {
                  ...
                  int16_sum = max<int32_t>(
                      int16_t::min,
                      min<int32_t>(
                          int16_t::max, int16_sum + x * w));
    ...
    #[cfg(use_fbgemm)]
    #[inline] pub fn dispatchfbgemm<PackAMatrix, const Q_GRAN: fbgemm::QuantizationGranularity>(&mut self, 
        packa:           &mut PackAMatrix,
        col_buffer_data: *const u8,
        y_int32:         *mut Vec<i32>,
        y_uint8_data:    *mut u8)  {
    ...
    #[inline] pub fn conv_outlier(&mut self, col_buffer: *const u8, y_int32: *mut Vec<i32>)  {
...
#[inline] pub fn conv_nhwc_acc16_ref(
    num_groups:        i32,
    n:                 i32,
          ...
          for (int j = 0; j < M / num_groups; ++j) {
            ...
            for (int k = 0; k < kernel_dim; ++k) {
              ...
              if (!overflowed && !underflowed) {
                if (int16_sum + x * w > int16_t::max) {
                  overflowed = true;
                } else if (int16_sum + x * w < int16_t::min) {
                  underflowed = true;
                }
              }
