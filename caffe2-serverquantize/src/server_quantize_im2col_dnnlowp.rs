...
#[inline] pub fn im_2col_nd_nchw<T>(
    n:            i32,
    img_size:     i32,
    col_size:     i32,
    img_shape:    *const i32,
    ...
    zero_point:   &T) 
{
    ...
    /*
        ...
        const int kernel_size = std::accumulate( kernel_shape, kernel_shape + N, 1, std::multiplies<int>());
...
/**
  ...
  | @note
  | 
  | groups are pulled out to an outer dimension
  | so that we can use GEMMs efficiently.
  |
  */
#[inline] pub fn im_2colNHWC<T>(
    channels:   i32,
    height:     i32,
    width:      i32,
    kernel_h:   i32,
