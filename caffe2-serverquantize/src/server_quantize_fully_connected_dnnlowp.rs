...
/**
 ...
 |
 | // x86 only provides SIMD instructions that
 | // multiply a signed integer with an unsigned
 | // integer. We use signed for weights.
 | //
 | using T_signed = typename make_signed<T>::type;
 */
pub struct FullyConnectedDNNLowPOp<T: GetSigned,const ReluFused: bool> {
    //USE_OPERATOR_FUNCTIONS(CPUContext);
    //USE_DNNLOWP_OPERATOR_BASE_FUNCTIONS(T, FullyConnectedOp<CPUContext>);
    base: DNNLowPOp<T, FullyConnectedOp<CPUContext,DefaultEngine,FCDefaultTransposeWeight>>,

    axis:                           usize, // default = 1
    axis_w:                         usize, // default = 1
    y_shape_cache:                  Vec<i64>,
    requantization_params:          Vec<RequantizationParams>,
    requantization_param_selected:  bool, // default = false

    /// used in fast path for T == uint8_t
    wq_packed:                      Arc<PackBMatrix<i8>>,

    x_pack_buf:                     Vec<u8>,
    y_int32:                        Vec<i32>,
    filter_qparams:                 Vec<TensorQuantizationParams>,
    filter_scales:                  Vec<f32>,
    filter_zero_points:             Vec<i32>,
    requantization_multipliers:     Vec<f32>,
    quantize_channelwise:           bool,

    /// used in slow path for T != uint8_t
    w_quantized:                    Vec<<T as GetSigned>::Type>,


    /// pre-computed biases and offsets
    b_quantized:                    Arc<Vec<i32>>,

    b_quantized_data:               *const i32, // default = nullptr
    row_offsets:                    Vec<i32>,
    column_offsets:                 Arc<Vec<i32>>,

    /// Dequantized bias populated when input bias is quantized and
    /// dequantized_output_ == true
    b_dequantized:                  Vec<f32>,

    b_dequantized_data:             *const f32, // default = nullptr
    is_weight_constant:             bool, // default = true
    in_qparams0_scale_old:          f32, // default = 0
    in_qparams0_zero_point_old:     i32, // default = 0

}

num_inputs!{Int8FCRelu, (3,4)}

num_outputs!{Int8FCRelu, 1}

cost_inference_function!{Int8FCRelu, /* (bind(CostInferenceForFC, _1, _2, false)) */ }

tensor_inference_function!{Int8FCRelu, /* (bind(FCShapeInference, _1, _2, false)) */}

register_cpu_operator_with_engine!{
    FC,
    DNNLOWP,
    FullyConnectedDNNLowPOp<u8>
}

register_cpu_operator_with_engine!{
    FC,
    DNNLOWP_16,
    FullyConnectedDNNLowPOp<u16>
}
...
register_cpu_operator_with_engine!{
    Int8FCRelu,
    DNNLOWP,
    FullyConnectedDNNLowPOp<u8, true>
}
...
declare_bool!{caffe2_dnnlowp_dump_tensors}

declare_bool!{caffe2_dnnlowp_force_slow_path}

impl<T: GetSigned, const ReluFused: bool> FullyConnectedDNNLowPOp<T, ReluFused> {
    ...
    pub fn new(operator_def: &OperatorDef, ws: *mut Workspace) -> Self {
        ...
        /*
          ...
          quantize_channelwise_(this->template GetSingleArgument<bool>( "quantize_channelwise", false)),
          b_quantized_(make_shared<vector<int32_t>>()),
          column_offsets_(make_shared<vector<int32_t>>()),
          is_weight_constant_( this->template GetSingleArgument<bool>("constant_weight", true)) 
    ...
    // #define DNNLOWP_DETAILED_LOG_IN_SLOW_PATH
        ...
        if (OutputSize() == 4) {
          ...
          X_q->Resize(vector<int64_t>{M, K});
          W_q->Resize(vector<int64_t>{N, K});
          Y_q->Resize(vector<int64_t>{M, N});
        ...
        if (!Wq_packed_) {
          ...
          for (int i = 0; i < M; ++i) {
            ...
            for (int j = 0; j < N; ++j) {
              ...
              if (ReluFused) {
                Ydata[i * N + j] = max(Ydata[i * N + j], 0.0f);
              }
            }
          }
        }
        ...
        if (!Wq_packed_) {
          ...
          for (int i = 0; i < M; ++i) {
            ...
            for (int j = 0; j < N; ++j) {
              ...
              if (ReluFused) {
                Ydata[i * N + j] =
                    max<T>(out_qparams_.zero_point, Ydata[i * N + j]);
              }
            }
          }
        }
    ...
    #[inline] pub fn get_quantization_parameters(&mut self) -> bool {
      ...
      if (!dequantize_output_ && !requantization_param_selected_) {
        ...
        if (InputSize() == 4) {
          ...
          TensorQuantizationParams out_qparams_overwrite;
