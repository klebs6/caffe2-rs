...
/**
  ...
  | 
  | ith bin (zero-based indexing) contains
  | [i*bin_width, (i+1)*bin_width)
  | 
  | with an exception that (nbins - 1)th
  | bin contains
  | 
...
impl Histogram {
    ...
    #[inline] pub fn get_histogram(&self) -> *const Vec<u64> {
    ...
    #[inline] pub fn add(&mut self, f: f32, cnt: Option<u64>) {

        let cnt: u64 = cnt.unwrap_or(1);
    ...
    /**
      | This version collects histogram with
      | single thread
      |
      */
    #[inline] pub fn add_collect_with_single_thread(&mut self, f: *const f32, len: i32)  {
...
/**
  | An equi-width histogram where the spread
  | of bins change over time when we see new
  | min or max values.
  |
  */
pub struct DynamicHistogram {
    ...
    /**
      | Dynamic histogram is implemented by the
      | series of static histograms and expands
      | from the old histogram to new histogram
      | when we see a new extremum.
      |
      | An invariant: the beginning of the first bin
      | of histograms_[i] exactly matches with the
      | beginning of a bin in
      | histograms_[i+1]. The end of the last bin
      | of histograms_[i] exactly matches with the
      | end of a bin in histograms_[i+1].
      */
    histogram:        Box<Histogram>,
    nbins:            i32,
    min:              f32,
    max:              f32,
    final_histogram:  Box<Histogram>,
}

pub const OVER_BINNING_FACTOR: i32 = 4;

#[inline] pub fn remap_histograms(src_hist: &mut Histogram, dst_hist: &mut Histogram)  {
...
impl DynamicHistogram {
    ...
    #[inline] pub fn add(&mut self, f: f32)  {
      ...
      if (f < curr_hist.Min() || f > curr_hist.Max()) {
        ...
        new_max = std::min(float::max, new_max);
        histogram_.reset(
    ...
    #[inline] pub fn add_collect_with_single_thread(&mut self, f: *const f32, len: i32)  {
      ...
      max_ = std::min(float::max, maximum);
    ...
    /**
      | Indicate we're not dynamically adjusting
      | histogram bins any more and return the
      | current static histogram.
      |
      */
    #[inline] pub fn finalize(&mut self) -> *const Histogram {
