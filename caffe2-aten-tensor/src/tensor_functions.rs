...
impl<'a> TensorMaker<'a> {

    pub type ContextDeleter = DeleterFnPtr;
    
    pub fn strides(&mut self, value: Option<&[i32]>) -> &'a mut TensorMaker {
    ...
    pub fn deleter(&mut self, value: fn(_0: *mut c_void) -> ()) -> &'a mut TensorMaker {
    ...
    pub fn context(
        &mut self, 
        value:   *mut c_void,
        deleter: Option<ContextDeleter>

    ) -> &'a mut TensorMaker {

        let deleter: ContextDeleter = deleter.unwrap_or(None);
    ...
    pub fn target_device(&mut self, value: Option<Device>) -> &'a mut TensorMaker {
    ...
    pub fn options(&mut self, value: TensorOptions) -> &'a mut TensorMaker {
...
#[inline] pub fn from_blob_a(
    ...
    let target_device = target_device.unwrap_or(None);
...
#[inline] pub fn from_blob_b(
    ...
    let options = options.unwrap_or(TensorOptions::default());
